...
A1(m0,DBG(lck++);Q(x);XP(0)Q(xr>0);P(--xr,0)Cb=xU;P(!b,x=AT(tn,x))xX=z[b];z[b]=(A)xV;XR(mrn(xn PR(+!xn),xA);x)x)
...
A aA0(Nn)_(Ax=AN(0,aA(n));PR(xx=oC);x)A1(aA1,Ay=an(tA,1);yx=x;y)A2(aA2,/*11*/a2t(x,y,tA))A3(aA3,/*111*/Au=an(tA,3);ux=x;uy=y;uz=z;u)
A2(aM,/*11*/a2t(x,y,tM))A2(am,/*11*/a2t(x,y,tm))AL(aA,Ax=an(tA,n);PR(I(!n,xx=oC));x)AL(aB,an(tB,n))AL(aC,an(tC,n))AL(aI,an(tI,n))AL(aL,an(tL,n))AL(aF,an(tF,n))AL(aS,an(tS,n))
...
V kinit(){S I l;P(l)l=1;z[LEN(z)-1]=1;A b[32],*c=b;i(tS-tA+1,*c++=ce[tA+i]=an(tA+i,0))*c++=ce[tm]=am(oS,oA);PR(_x(ce[tA])=oC);
...
#define xys(a...) obs(I(xtR&&xr,i(xn PR(+!xn),Ay=xa;a)))
