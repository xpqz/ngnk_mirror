...
AQ(aCz,aCn(s,SL(s)))
A az(L n)_(n-(I)n?al(n):ai(n))A al(L v)_(aV(tl,1,&v))A af(F v)_(aV(tf,1,&v))A aE(Li,Lj)_(Q(i<=j)P(i==j,oG)Ax=an(tE,2);*xL=i;xL[1]=j;x)
A1(mut,XP(x)P(xr==1,x)x=x(aV(xt,xn,xV));XR(mRa(x))x)C tZ(L v)_(tL-((v==(G)v)+(v==(H)v)+(v==(I)v)))A kv(A*p)_(Ax=*p;Q(xn==2);P(xr>1,--xr;*p=_R(xx);_R(xy))*p=xx;AZ(x);x(xy))
A room(L n,A x)_(P(xr==1&&n*xW<=xZ,AN(n,x))Ay=an(n,xt);MC(yV,xV,xn*xW);I(ytR,I(xr==1,AZ(x))E(i(xn,_R(ya))))x(y))L gl_(Ax)_(XP(xv)*xL)L gl(Ax)_(L v=gl_(x);x(0);v)F gf(Ax)_(F v=*xF;x(0);v)
...
S I sq(Qs)_(Nn=SL(s);P(n<4||(n==4&&!(s[3]&128)),I v=0;MC(&v,s,n);v)Qp=s0+1;W(p<s1,P(!strcmp(p,s),s0-p)p+=SL(p)+1)n++;P(s1+n>s0+SZ s0,die("SYMS"))MC(s1,s,n);s1+=n;s0-s1+n)
AQ(sym,as(sq(s)))

S AQ(bs0,en0())
S AQ(bsbs,exit(0);0)
S AQ(bscd,P(!*s,C b[256];getcwd(b,SZ b)?eo0():aCz(b))chdir(s)?eo0():au)
...
SN V o8(W v){C b[16],*s=b;i(16,Cc=v>>4*(15-i)&15;*s++="0W"[9<c]+c)ow(b,16);}
