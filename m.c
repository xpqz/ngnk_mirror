...
S AQ(bsd,P(!*s,aCz(gp))s+=*s=='.';Nn=SL(s);P(n+2>SZ(gp),ez0())MC(gp,s,n+1);au)
  AQ(bsl,Ax=N(u1c(aCz(s)));P(!xn,au)C*p=xC,*e=p+xn;P(e[-1]-10,x(err0("eoleof")))e[-1]=0;I(*p=='#'&&p[1]=='!',p=strchrnul(p,10);p+=!!*p)evs(p,1);au)
...
S A evC1(C**p)_(C*s=*p;
  ...
  *p=q;x)
 Ax=pk((V*)p);C*e=*p;*p+=!!*e;N(x);x=N(cpl(aCm(s,e),x,oS));x(run(x,0,0)))
A evC(C*s,I b)_(W(*s,Ax=evC1(&s);I(b,x?x(out(x)):epr(0))E(N(x);P(!*s,x)x(0))mc())au)
