...
S A o1f(A1 f,Ax/*f1*/)_(f==len?x(ai(0)):f==str?x(oC):f==cS||f==enl?f(x):x)S A o2f(A2 f,Ax,Ay/*f01*/)_(f==bng?y(ai(0)):arf2(f)||f==hsh||f==cst?f(x,y):y(oC))
S A2(o1,/*01*/xtu?o1f(v1[xv],y):y(oC))S A3(o2,/*001*/xtv?o2f(v2[xv],y,z):z(oC))
S AX(em,/*01..1*/Az=oS;Ik=0;i(n,Ay=a[i];I(ytm,k++;z=cat10(z,yx)))I(k>1,z=unq(z))Ab8;i(n,Ay=b[i]=a[i];I(ytm,Au=kv(&y);uq(ie(x,u));b[i]=u(u1(y(fil(ai(yn),fnd(y,zR)))))))AX e8;x=Nz(e8(x,b,n));z(dct(z,x)))
...
A1(raz,XA(Nn=0;i(xn,n+=_N(xa))Ay=xx;y=ytT&&!ytA?AN(0,an(n,yt)):aA0(n);i(xn,y=Nx(cat10(y,xa)))x(y))Xm(raz(val(x)))x)
...
S A2(binZ,/*01*/Q(xtZC)
 YZC(P(xn-(I)xn,ez1(y))I wx=xw,wy=yw;P(!wx,wy?K2("{@[x'`c$127&y;&-128>y;:;-1]}",x,y):K2("{(-1+\\@[&256;128+x;+;1])128+y}",x,y))Az=an(yn,tZ(xn-1));I wz=zw,k[2];
  i(yn,Lv;S4(wy,v=yb,v=yh,v=yi,v=yl)*k=-1;k[1]=xn;S4(wx,,W(*k+1<k[1],Ii=*k+k[1]>>1;k[v<xh]=i),W(*k+1<k[1],Ii=*k+k[1]>>1;k[v<xi]=i),W(*k+1<k[1],Ii=*k+k[1]>>1;k[v<xl]=i))
  S4(wz,zb=*k,zh=*k,zi=*k,zl=*k))y(z))
 YF(x=cF(xR);x(binF(x,y)))Yt(YU(ed1(y))fir(N(binZ(x,enl(y)))))YmMA(r2f(binZ,x,y))ed1(y))
S A2(binF,/*01*/Q(xtF)YF(x=of1(xR);x(binZ(x,of1(y))))YZC(binF(x,N(cF(y))))Yt(YU(ed1(y))fir(N(binF(x,enl(y)))))YmMA(r2f(binF,x,y))ed1(y))
S A2(bin,/*01*/XZC(binZ(x,y))XF(binF(x,y))Xm(_1(xx,N(bin(xy,y))))et1(y))
A2(w1,/*01*/Q(xtr)Ii=xE;x=xx;SW(i,R(0,Xt(e1(x,y))bin(x,y))R(1,P(xtzZ||xtfF,dec(x,y))Xc(jc(xv,y))XC(jC(xV,xn,y))P(xK<2,cf(x,y))f1(x,y))
 R(2,XfF(en1(y))XzZ(enc(x,y))Xc(sc(xv,y))XC(sC(xV,xn,y))P(xK<2,cs(x,y))s1(x,y))R(3,Xz(win(gl_(x),y))p1(x,y))R(4,er1(y))R(5,e1(x,y)))en1(y))
A3(w2,/*010*/Q(xtr)Ii=xE;x=xx;SW(i,R(0,y(e2(x,y,zR)))R(1,(xK<2?lf2:f2)(x,y,z))R(2,(xK<2?ls2:s2)(x,y,z))R(3,(xK==1?ste:p2)(x,y,z))R(4,y(r2(x,y,zR)))R(5,y(l2(x,y,zR))))en1(y))
AX(w8,/*01..1*/Q(xtr)Ay=*a;P(n==1,w1(x,y))P(n==2,Az=a[1];z(w2(x,y,z)))Ii=xE;x=xx;SW(i,R(0,e8(x,a,n))R(1,P(xK<2||n-xK==1,lf8(x,a,n))f8(x,a,n))
 R(2,P(xK<2||n-xK==1,ls8(x,a,n))s8(x,a,n))R(3,er8(a,n))R(4,er8(a,n))R(5,l8(x,a,n)))en8(a,n))
