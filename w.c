...
A e1f(A1 f,Ax){X(Rt(f(x))Rm(Ay=kv(&x);am(x,Nx(e1f(f,y))))
 RA(Nn=xn;P(!n,re0(N(o1f(f,fir(x)))))P(xr==1,i(n,P(!(xa=f(xa)),xa=au;x(0)))sqz(x))Ay=aA0(n);i(n,Az=f(_R(xa));B(!z,y=y(0))yq(z))x(y))
 R_(Nn=xN;P(!n,re0(N(o1f(f,fir(x)))))Ay=aA0(n);i(n,Az=f(ii(x,i));B(!z,y=y(0))yq(z))x(y)))}
S A2(e1,/*01*/Xu(e1f(v1[xv],y))
 Y(Rt(x1(y))Rm(em(x,&y,1))
  RA(Nn=yn;P(!n,re0(N(o1(x,fir(y)))))P(yr==1,i(n,;P(!(ya=x1(ya)),xa=au;y(0)))sqz(y))Au=aA0(n);i(n,Az=x1(_R(ya));B(!z,u=u(0))uq(z))y(u))
  R_(Nn=yN;P(!n,re0(N(o1(x,fir(y)))))Au=aA0(n);i(n,Az=x1(ii(y,i));B(!z,u=u(0))uq(z))y(u)))0)
A l2f(A2 f,Ax,Ay/*f01*/)_(Xt(f(x,y))Xm(Az=N(l2f(f,xy,y));       am(_R(xx),z) )     Nn=xN;P(!n,x=fir(xR);re0(N(x(o2f(f,x,y  )))))Au=aA0(n);i(n,A w=ii(x,i),v=f(w,yR);mr(w); B(!v,u=u(0))uq(v))y(u))
A r2f(A2 f,Ax,Ay/*f01*/)_(Yt(f(x,y))Ym(Az=Ny(r2f(f,x,_R(yy)));y(am(_R(yx),z)))     Nn=yN;P(!n,          re0(N(o2f(f,x,fir(y)))))Au=aA0(n);i(n,A v=f(x,ii(y,i));            B(!v,u=u(0))uq(v))y(u))
