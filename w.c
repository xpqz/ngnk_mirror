...
S A emp1f(Ax,A1 f/*0f*/)_(f==len?oB:f==til?oA:xR)S A emp1(Ax,Ay/*000*/)_(ytu?emp1f(x,v1[yv]):oA)
...
NI A eal2f(Ax,Ay,A2 f/*11f*/)_(Xt(f(x,y))Xm(Az=kv(&x);am(x,Ny(eal2f(z,y,f))))Nn=xN;Au=emp2f(x,y,f);i(n,Az=f(ii(x,i),yR);B(!z,u=u(0))uq(z))x(y(u)))
NI A ear2f(Ax,Ay,A2 f/*11f*/)_(Yt(f(x,y))Ym(Az=kv(&y);am(y,Nx(ear2f(x,z,f))))Nn=yN;Au=emp2f(x,y,f);i(n,Az=f(xR,ii(y,i));B(!z,u=u(0))uq(z))x(y(u)))
