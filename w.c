...
S A emp1f(Ax,A1 f/*0f*/)_(f==len?oB:f==til||f==str?oA:xR)S A2(emp1,/*00*/ytu?emp1f(x,v1[yv]):oA)
S A emp2f(Ax,Ay,A2 f/*00f*/)_(f==bng?oB:oA)              S A3(emp2,/*000*/ztv?emp2f(x,y,v2[zv]):oA)
SN AX(eacm,Az=oS;Ik=0;i(n,Ay=a[i];I(ytm,k++;z=cat(z,_R(yx))))I(k>1,z=unq(z))Ab8;i(n,Ay=b[i]=a[i];I(ytm,Au=kv(&y);uq(id_(x,u));b[i]=ap1(u,fil(ai(yn),fnd(y,zR)))))AX eac;x=eac(x,b,n);P(!x,z(x))dct(z,x))
A eac1f(Ax,A1 f)_(Xt(f(x))Xm(Ay=kv(&x);am(x,Nx(eac1f(y,f))))I b=xtA&&xr==1;Nn=xN;P(!n,x(emp1f(x,f)))Ay=oA;i(n,Az=f(b?xa:ii(x,i));B(!z,I(b,mrn(xn-i-1,xA+i+1))y=y(0))yq(z))I(b,x=AN(0,x))x(y))
SN A2(eac1,/*10*/Yu(eac1f(x,v1[yv]))Xt(y1(x))Xm(eacm(y,&x,1))Nm=xN;P(!m,x(emp1(x,y)))P(xtA&&xr==1,x=AN(0,x);Au=oB;i(m,Az=y1(xa     );B(!z,u=u(0);mrn(m-i-1,xA+i+1))uq(z))x(u))
                                                                                                            Au=oB;i(m,Az=y1(ii(x,i));B(!z,u=u(0)                  )uq(z))x(u))
NI A eal2f(Ax,Ay,A2 f/*11f*/)_(           Xt( f(x,y))Xm(Az=kv(&x);am(x,Nx(eal2f(z,y,f))))Nn=xN;P(!n,x(y(emp2f(x,y,f))))Au=oA;i(xN,A v= f(ii(x,i),yR);B(!v,u=u(0))uq(v))x(y(u)))
NI A ear2f(Ax,Ay,A2 f/*11f*/)_(           Yt( f(x,y))Ym(Az=kv(&y);am(y,Ny(ear2f(x,z,f))))Nn=yN;P(!n,x(y(emp2f(x,y,f))))Au=oA;i(yN,A v= f(xR,ii(y,i));B(!v,u=u(0))uq(v))x(y(u)))
SN A3(eal2,/*110*/P(ztv,eal2f(x,y,v2[zv]))Xt(z2(x,y))Xm(z=prj(z,A(PLH,y),2);z(eac1(x,z)))Nn=xN;P(!n,x(y(emp2 (x,y,z))))Au=oA;i(xN,A v=z2(ii(x,i),yR);B(!v,u=u(0))uq(v))x(y(u)))
NI A3(ear2,/*110*/P(ztv,ear2f(x,y,v2[zv]))Yt(z2(x,y))Ym(z=prj(z,A(x,PLH),2);z(eac1(y,z)))Nn=yN;P(!n,x(y(emp2 (x,y,z))))Au=oA;i(yN,A v=z2(xR,ii(y,i));B(!v,u=u(0))uq(v))x(y(u)))
SN AX(eal,Ab8;Mc(b,a,8*n);*b=PLH;x=prj(x,b,n);x(eac1(*a,x)))
SN A3(eac2,/*110*/P(ztv,eac2f(x,y,v2[zv]))Xt(ear2(x,y,z))Yt(eal2(x,y,z))P(xtm||ytm,eacm(z,A(x,y),2))Nm=xN;P(m-yN,el2(x,y))P(!m,y(ztv&&zv<11?x:x(oA)))Au=0;Ct=xtA&&xr==1,s=ytA&&yr==1;
...
AX(eac,P(n==1,eac1(*a,x))P(n==2,eac2(*a,a[1],x))Ab8;Ct[8];Lm=-1;i(n,Ay=b[i]=a[i];Ym(eacm(x,a,n))t[i]=ytP?0:ytt?1:ytA?2+(yr>1):4;I(t[i]>1,L l=yN;P(m>=0&&m-l,el8(a,n))m=l))
...
AX(fld,P(n==1,fld1(*a,x))P(n==2,fld2(*a,a[1],x))n--;Ay=*a++,z=*a;Lm=cfm(a,n);P(m==-1,y?x8(a-1,n+1):z)P(m<0,I(y,y(0))el8(a,n))P(!m&&!y,x=id_(x,z);mrn(n,a);x)
 Li=!y;I(i,y=ii(z,0))Ab8;W(i<m,*b=y;j(n,b[j+1]=ii(a[j],i))y=x8(b,n+1);B(!y)i++)mrn(n-1,a+1);z(y))
SN A3(scn2,/*110*/Yt(z2(x,y))Ym(Au=kv(&y);am(y,Ny(scn2(x,u,z))))P(!yN,x(y))P(ztv&&zv<11&&xtzc&&ytZC,ars(x,y,z))Au=oA;i(yN,x=z2(x,ii(y,i));P(!x,y(u(0)))uq(xR))x(y(u)))
SN A2(scn1,/*10*/Xt(x)P(!xN,x)Xm(Az=kv(&x);am(x,Nx(scn1(z,y))))P(y==CAT,scn2(oA,x,y))P(ytv&&yv<11&&xtZC,ars(0,x,y))Ni=0,n=xN;Az=ii(x,0),u=enl(zR);W(++i<n,z=N(y2(z,ii(x,i)));uq(zR))z(x(u)))
SN AX(scn,P(n==1,scn1(*a,x))P(n==2,scn2(*a,a[1],x))Ay=*a++;n--;Lm=cfm(a,n);P(m==-2,y(el8(a,n)))Az=oA;Ab8;i(m,*b=y;j(n,b[j+1]=ii(a[j],i))y=x8(b,n+1);P(!y,mrn(n,a);z(0))zq(yR))mrn(n,a);y(z))
SN A3(eap2,/*110*/Ym(Au=kv(&y);am(y,Ny(eap2(x,u,z))))Yt(et2(x,y))P(!yN,x(y))P(ztv&&zv<11&&xtzc&&ytZC,arp(x,y,z))Au=oA;i(yN,x=z2(ii(y,i),x);P(!x,y(u(0)))uq(x);x=ii(y,i))x(y(u)))
SN A2(eap1,/*10*/eap2(id_(y,x),x,y))
SN A3(ste,/*110*/K("{z'x':y}",x,y,zR))
SN A2(bin,/*01*/Xm(_1(xx,bin(xy,y)))Q(xtZC)
 YZC(P(xn-(I)xn,ez1(y))I wx=xw,wy=yw;P(!wx,wy?K("{@[x'`c$127&y;&-128>y;:;-1]}",xR,y):K("{(-1+\\@[&256;128+x;+;1])128+y}",xR,y))Az=an(tZ(xn-1),yn);I wz=zw,k[2];
  i(yn,Lv;S4(wy,v=yb,v=yh,v=yi,v=yl)*k=-1;k[1]=xn;S4(wx,,W(*k+1<k[1],Ii=*k+k[1]>>1;k[v<xh]=i),W(*k+1<k[1],Ii=*k+k[1]>>1;k[v<xi]=i),W(*k+1<k[1],Ii=*k+k[1]>>1;k[v<xl]=i))S4(wz,zb=*k,zh=*k,zi=*k,zl=*k))y(z))
 Yt(fir(bin(x,enl(y))))YMT(ear2f(x,y,bin))ed1(y))
AX(adv,Q(xtr)Ii=xE;x=xx;Ik=xK;SW(i,
 CR(0,P(n==1,Xt(eac1(*a,x))XZC(bin(x,*a))Xm(bin(x,*a))ed1(*a))eac(x,a,n))
 CR(1,P(n==1,Ay=*a;P(xtzZ||xtdD,dec(x,y))Xc(jc(xv,y))XC(jC(xV,xn,y))P(k<2,cf(x,y))fld1(y,x))P(k<2,nwf(x,a,n))P(n-k==1,nwf(x,a,n))fld(x,a,n))
 CR(2,P(n==1,Ay=*a;P(xtzZ||xtdD,enc(x,y))Xc(sc(xv,y))XC(sC(xV,xn,y))P(k<2,cs(x,y))scn1(y,x))P(k<2,nws(x,a,n))P(n-k==1,nws(x,a,n))scn(x,a,n))
 CR(3,n==1?xtz?win(gl(x),*a):eap1(*a,x):n==2?k==1?ste(*a,a[1],x):eap2(*a,a[1],x):er8(a,n))
 CR(4,P(n==2,ear2(*a,a[1],x))er8(a,n))
 CR(5,P(n==2,eal2(*a,a[1],x))eal(x,a,n)))er8(a,n))
