...
A e1f(A1 f,A x){X(Rt(f(x))Rm(A y=kv(&x);am(x,Nx(e1f(f,y))))
 ...
 RZ_E(N n=xn;P(!n,o1f(f,x))A y=aA(n);S4(xt-tG,i(n,P(!(ya=f(ai(xg))),mrn(i,yA);x(0))),
...
S A nS(A x,L m,OA*a,N n/*0m1n*/)_(P(n==1,ns(x,m,*a))P(m<0,mrn(n,a);ed0())P(m<n,mrn(n-m-1,a+m+1);sqz(aV(tA,m+1,a)))
 A z=aA(m+n),*b=zA;zn=n;MC(zA,a,8*n);i(m+1-n,mRn(n,b);b[n]=Nz(x8(b,n));zn++;b++)sqz(z))
S A wS(A x,A y,OA*a,N n/*001n*/)_(P(n==1,ws(x,y,*a))Ab8;MC(b,a,8*n);mRn(n,b);A z=sqz(aV(tA,n,b));
 W(1,A u=y1(ii(z,zn-1));B(!u,z=z(0))B(!tru(u))mRn(n-1,b+1);u=x8(b,n);B(!u,*b=au;z=z(0))memmove(b,b+1,8*n-8);b[n-1]=u;zq(uR))mrn(n,b);z)
S A nF(A x,L m,OA*a,N n/*0m1n*/)_(P(n==1,nf(x,m,*a))las(N(nS(x,m,a,n))))
S A wF(A x,A y,OA*a,N n/*001n*/)_(P(n==1,wf(x,y,*a))las(N(wS(x,y,a,n))))
...
L cfm(OA*a/*0*/,I n)_(L m=-1;i(n,A x=a[i];I(!xtt,U v=xN;P(m>=0&&m-v,-2)m=v))m)
...
A w8(U i,A x,OA*a,U n/*0,1..1*/){A y=*a;P(n==1,w1(i,x,y))P(n==2,A z=a[1];z(w2(i,x,y,z)))
