...
A jc(C c,    A x)_(XC(jc(c,  flp(flp(x))))Ln=jN(1,x);P(n<0,et1(x))Ay=aC(n);C*p=yV;i(xn,I(i,*p++=c        )Az=xa;I(ztc,*p++=zv)E(MC(p,zV,zn);p+=zn))x(y))
A jC(Q s,U m,A x)_(XC(jC(s,m,flp(flp(x))))Ln=jN(m,x);P(n<0,et1(x))Ay=aC(n);C*p=yV;i(xn,I(i,MC(p,s,m);p+=m)Az=xa;I(ztc,*p++=zv)E(MC(p,zV,zn);p+=zn))x(y))

S A1(re0,x?rs0(enl(x)):0) S A o1f(A1 f,Ax/*f1*/)_(re0(f==whr?x(aI(0)):f(fir(x)))) S A2(o1,/*01*/ xtu?o1f(v1[xv],y):y(oA))
...
S A3(l2,/*001*/Xv(l2f(v2[xv],y,z))Yt(x2(y,z))Ym(x=prj(x,A((A)GAP,z),2);x(e1(x,yR)))Nn=yN;P(!n,y=fir(yR); y(o2(x,y,z)))Au=aA0(n);i(n,A w=ii(y,i),v=x2(w,zR);mr(w);B(!v,u=u(0))uq(v))z(u))
  A3(r2,/*001*/Xv(r2f(v2[xv],y,z))Zt(x2(y,z))Zm(x=prj(x,A(yR,GAP),2);x(e1(x,z)))   Nn=zN;P(!n,         o2(x,y,fir(z)))Au=aA0(n);i(n,A v=x2(y,ii(z,i));           B(!v,u=u(0))uq(v))z(u))
S AX(l8,/*01..1*/Ab8;MC(b,a,8*n);*b=GAP;x=prj(x,b,n);x(e1(x,*a)))
  A3(e2,/*001*/Xv(e2f(v2[xv],y,z))Yt(r2(x,y,z))Zt(l2(x,y,z))P(ytm||ztm,em(x,A(yR,z),2))Nm=yN;P(m-zN,el1(z))P(!m,z(xtv&&xv<11?yR:oA))Au=0;Ct=ztA&&zr==1;
...
A e2f(A2 f,Ax,Ay/*f01*/)_(Ik=xtt<<1|ytt;P(k==3,f(x,y))
 P(xtm||ytm,P(xtm>ytm,Az=N(e2f(f,xy,y));am(_R(xx),z))P(xtm<ytm,Az=Ny(e2f(f,x,_R(yy)));y(am(_R(yx),z)))
  Az=unq(cat(xx,_R(yx)));x=x1(zR);y=y(y1(zR));I(f==add||f==sub||f==mul||f==dvd,Au=ai(f==mul||f==dvd);x=fil(u,x);y=fil(u,y))am(z,Nz(x(e2f(f,x,y)))))
 P(!k&&xN-yN,el1(y))Nn=k<2?xN:yN;P(!n,x=fir(xR);x(o2f(f,x,fir(y))))Az=oA;i(n,A v=ii(x,i);Au=f(v,ii(y,i));mr(v);B(!u,z=z(0))zq(u))y(z))
...
S A nS(Ax,Lm,OA*a,Nn/*0m1n*/)_(P(n==1,ns(x,m,*a))P(m<0,mrn(n,a);ed0())P(m<n,mrn(n-m-1,a+m+1);sqz(aV(tA,m+1,a)))
 Az=aA(m+n),*b=zA;zn=n;MC(zA,a,8*n);i(m+1-n,mRn(n,b);b[n]=Nz(x8(b,n));zn++;b++)sqz(z))
...
S A3(ste,/*010*/Yz(stn(x,gl(y),z))et1(y))
S A win(L n,A x)_(x(stn(au,n,x)))
S AX(cas,/*01..1*/Q(xtZ)K2("{(++y)[x]@'!#x}",x,aV(tA,n,a)))
A w1(U i,A x,Ay/*01*/){SW(i,R(0,Xt(e1(x,y))bin(x,y))R(1,P(xtzZ||xtfF,dec(x,y))Xc(jc(xv,y))XC(jC(xV,xn,y))P(xK<2,cf(x,y))f1(x,y))
...
A w8(U i,A x,OA*a,U n/*0,1..1*/){Ay=*a;P(n==1,w1(i,x,y))P(n==2,Az=a[1];z(w2(i,x,y,z)))
