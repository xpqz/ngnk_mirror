...
AX e8;
S AX(em,/*01..1*/
 A z=0;U k=0;i(n,A y=a[i];I(ytm,k++;y=yx;I(z,P(TS[yt]-TS[zt],ed8(a,n));z=cat10(z,y))E(z=yR)))
 Q(z)I(k>1,z=unq(z))
 Ab8;MC(b,a,8*n);
 i(n,A y=b[i];I(ytm,A u=kv(&y);uq(ie(x,u));b[i]=u(u1(y(fil(ai(yn),fnd(y,zR)))))))
 am(z,Nz(e8(x,b,n))))
...
A e1f(A1 f,A x){X(Rt(f(x))Rm(A y=kv(&x);am(x,Nx(e1f(f,y))))
 ...
 RGHIL(N n=xn;P(!n,o1f(f,x))A y=aA(n);S4(xt-tG,i(n,P(!(ya=f(ai(xg))),mrn(i,yA);x(0))),
                                               i(n,P(!(ya=f(ai(xh))),mrn(i,yA);x(0))),
                                               i(n,P(!(ya=f(ai(xi))),mrn(i,yA);x(0))),
                                               i(n,P(!(ya=f(az(xl))),mrn(i,yA);x(0))))sqz(x(y)))
 R_(N n=xN;P(!n,o1f(f,x))A y=aA0(n);i(n,A z=f(ii(x,i));B(!z,y=y(0))yq(z))x(y)))}
S A2(e1,/*01*/Xu(e1f(v1[xv],y))Y(Rt(x1(y))Rm(A z=kv(&y);am(y,Ny(e1(x,z))))
 RA(N n=yn;P(!n,o1(x,y))y=mut(y);   i(n,;P(!(ya=x1(ya)),ya=au;y(0)))sqz(y))
 R_(N n=yN;P(!n,o1(x,y))A u=aA0(n); i(n,A z=x1(ii(y,i));B(!z,u=u(0))uq(z))y(u)))0)
A l2f(A2 f,A x,A y/*f01*/)_(Xt(f(x,y))Xm(A z=N(l2f(f,xy,y));       am(_R(xx),z) )  N n=xN;P(!n,x=fir(xR);x(o2f(f,x,y)))A u=aA0(n);i(n,A w=ii(x,i),v=f(w,yR);mr(w); B(!v,u=u(0))uq(v))y(u))
A r2f(A2 f,A x,A y/*f01*/)_(Yt(f(x,y))Ym(A z=Ny(r2f(f,x,_R(yy)));y(am(_R(yx),z)))  N n=yN;P(!n,        o2f(f,x,fir(y)))A u=aA0(n);i(n,A v=f(x,ii(y,i));            B(!v,u=u(0))uq(v))y(u))
S A3(l2,/*001*/Xv(l2f(v2[xv],y,z))Yt(x2(y,z))Ym(x=prj(x,A((A)GAP,z),2);x(e1(x,yR)))N n=yN;P(!n,y=fir(yR); y(o2(x,y,z)))A u=aA0(n);i(n,A w=ii(y,i),v=x2(w,zR);mr(w);B(!v,u=u(0))uq(v))z(u))
  A3(r2,/*001*/Xv(r2f(v2[xv],y,z))Zt(x2(y,z))Zm(x=prj(x,A(yR,GAP),2);x(e1(x,z)))   N n=zN;P(!n,         o2(x,y,fir(z)))A u=aA0(n);i(n,A v=x2(y,ii(z,i));           B(!v,u=u(0))uq(v))z(u))
S AX(l8,/*01..1*/Ab8;MC(b,a,8*n);*b=GAP;x=prj(x,b,n);x(e1(x,*a)))
  A3(e2,/*001*/Xv(e2f(v2[xv],y,z))Yt(r2(x,y,z))Zt(l2(x,y,z))P(ytm||ztm,em(x,A(yR,z),2))N m=yN;P(m-zN,el(z))P(!m,z(xtv&&xv<11?yR:oA))A u=0;C t=ztA&&zr==1;
...
A e2f(A2 f,A x,A y/*f01*/)_(I k=xtt<<1|ytt;P(k==3,f(x,y))
 ...
 P(!k&&xN-yN,el(y))N n=k<2?xN:yN;P(!n,x=fir(xR);x(o2f(f,x,fir(y))))A z=oA;i(n,A v=ii(x,i);A u=f(v,ii(y,i));mr(v);B(!u,z=z(0))zq(u))y(z))
...
S A2(cs,/*01*/A z,v=yR,u=enl(yR);W(1,z=yR;y=x1(y);P(!y,mr(v);z(u(0)))I m=mtc_(y,z)||mtc_(y,v);z(0);B(m)uq(yR))mr(v);y(u))
...
S A nS(A x,L m,O A*a,N n/*0m1n*/)_(P(n==1,ns(x,m,*a))P(m<0,mrn(n,a);ed0())P(m<n,mrn(n-m-1,a+m+1);sqz(aV(tA,m+1,a)))
 A z=aA(m+n),*b=zA;zn=n;MC(zA,a,8*n);i(m+1-n,mRn(n,b);b[n]=Nz(x8(b,n));zn++;b++)sqz(z))
S A wS(A x,A y,O A*a,N n/*001n*/)_(P(n==1,ws(x,y,*a))Ab8;MC(b,a,8*n);mRn(n,b);A z=sqz(aV(tA,n,b));
 W(1,A u=y1(ii(z,zn-1));B(!u,z=z(0))B(!tru(u))mRn(n-1,b+1);u=x8(b,n);B(!u,*b=au;z=z(0))memmove(b,b+1,8*n-8);b[n-1]=u;zq(uR))mrn(n,b);z)
S A nF(A x,L m,O A*a,N n/*0m1n*/)_(P(n==1,nf(x,m,*a))las(N(nS(x,m,a,n))))
S A wF(A x,A y,O A*a,N n/*001n*/)_(P(n==1,wf(x,y,*a))las(N(wS(x,y,a,n))))
S A3(ls2,/*010*/Y(Ril(ns(x,gl(y),zR))RU(y(ws(x,y,zR)))R_(et(y)))0)
S A3(lf2,/*010*/Y(Ril(nf(x,gl(y),zR))RU(y(wf(x,y,zR)))R_(et(y)))0)
S AX(ls8,/*01..1*/A y=*a;P(n==2,A z=a[1];z(ls2(x,y,z)))Yz(nS(x,gl(y),a+1,n-1))YU(y(wS(x,y,a+1,n-1)))et8(a,n))
S AX(lf8,/*01..1*/A y=*a;P(n==2,A z=a[1];z(lf2(x,y,z)))Yz(nF(x,gl(y),a+1,n-1))YU(y(wF(x,y,a+1,n-1)))et8(a,n))
X1(raz,RA(N n=0;i(xn,n+=_N(xa))A y=xx;y=ytT&&!ytA?AN(0,an(n,ytE?tG:yt)):aA0(n);i(xn,y=Nx(cat10(y,xa)))x(y))Rm(raz(val(x)))R_(x))
...
A w1(U i,A x,A y/*01*/){SW(i,R(0,Xt(e1(x,y))bin(x,y))R(1,P(xtzZ||xtfF,dec(x,y))Xc(jc(xv,y))XC(jC(xV,xn,y))P(xK<2,cf(x,y))f1(x,y))
 R(2,XfF(en(y))XzZ(enc(x,y))Xc(sc(xv,y))XC(sC(xV,xn,y))P(xK<2,cs(x,y))s1(x,y))R(3,Xz(win(gl_(x),y))p1(x,y))R(4,er(y))R_(e1(x,y)))}
A w2(U i,A x,A y,A z/*010*/){SW(i,R(0,XZ(cas(x,A(y,zR),2))y(e2(x,y,zR)))R(1,(xK<2?lf2:f2)(x,y,z))R(2,(xK<2?ls2:s2)(x,y,z))R(3,(xK==1?ste:p2)(x,y,z))R(4,y(r2(x,y,zR)))R_(y(l2(x,y,zR))))}
A w8(U i,A x,O A*a,U n/*0,1..1*/){A y=*a;P(n==1,w1(i,x,y))P(n==2,A z=a[1];z(w2(i,x,y,z)))
