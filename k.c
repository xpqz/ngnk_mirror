...
N NK(K x)_(xn)
V*dK(K x)_(xV)
S V VK(C t,V*a,K x){I(xtZ,x=cT[t](x))MC(a,xV,xn*TW[t]);I(xtR,I(xr==1,AN(0,x))E(mRn(xn,xV)))x(0);}

//            constructors                                                     accessors
//    atom                     list                                    atom                 list
K Kc(C v)_(ac(v))    K KC(C*a ,N n)_(aV(tC,n,a))                   C cK(K x)_(xv)  V CK(C*a,K x){VK(tC,a,x);}
K Ks(C*s)_(sym(s))   K KS(C**a,N n)_(Ax=aS(n);i(n,xi=sym(a[i]))x)
