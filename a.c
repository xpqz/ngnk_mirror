...
A1(val,XS(P(!xn,I*k=gk;Nm=gn;Ax=oS,y=oA;i(gn,I(gv[i],xq(as(k[i]));yq(_R(gv[i]))))am(x,y))K(".`$\".\"/$:",x))
...
A mnd(OA*a,In)_(Ax=*a;Q(xtsS)I(xtS,x=K("`$\".\"/$:",x))Li=fI(gk,gn,xv);I(i<0,gk[i=gn++]=xv)A*p=gv+i;I(!*p,*p=au)
 Ab8;*b=*p;Mc(b+1,a+1,(n-1)*8);*p=au;*p=_R(N(dm8(b,n))))
AA(dmd,Ax=*a;XsS(mnd(a,n))I(n==3,XF(try(x,a[1],a[2])))dm8(a,n))
