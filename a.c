...
X1(mkn,RmMA(e1f(mkn,x))Rt(x(_R(cn[xt])))R_(xN?rsz(xN,mkn(fir(x))):x))
A1(iei,/*0*/xtv&&xv<10u?"\4\0\0\1\1\2\2\3\3\2"[xv]:4)
Y2(iex,/*01*/RmMA(r2f(iex,x,y))RT_(rsz(yN,iex(x,fir(y))))Rc(ac("\0\1\x7f\x80 "[iei(x)]))Rf(y(af(A(0.,1.,WF,-WF,NF)[iei(x)])))R_(y(az(A(0ll,1,WL,-WL,NL)[iei(x)]))))
...
AAL(ii,/*0*/SW(xt,RA(_R(xa))RC(ac(xc))RB(ai(xb))RH(ai(xh))RI(ai(xi))RL(al(xl))RF(af(xf))RS(as(xi))Rm(ii(xy,i))RM(iM(x,i,ii))R_(xR))0)
AAL(io,/*0*/SW(xt,RT(IN(i,xn)?ii(x,i):xn?mkn(ii(x,0)):xtA?_R(xx):_R(cn[xt]))Rt(xR)Rm(io(xy,i))RM(iM(x,i,io)))0)A1(fir,x(io(x,0)))A1(las,x(io(x,xN-1)))
#define G(T,U) (32/(SZ(T)>SZ(U)?SZ(T):SZ(U)))
#define iF(f,T,U,R,e...) S L f(OV*RES a,Lm,OV*RES b,V*RES c,Nn)_(O T*p=a;O U*q=b;R*r=c;LI(p)LI(q)LI(r)e;n)
...
A2(i1,/*01*/Xt(y(xR))P(y==PLH||y==au,xR)P(xtm||xtM&&ytsS,i1(xy,N(fnd(xx,y))))Yzc(io(x,gl(y)))Yt(et1(y))YmM(Az=kv(&y);am(y,Ny(i1(x,z))))YA(r2(AP1,x,y))
 ...
 Az=xtL&&ytL&&yr==1?y:an(n,xt);Lm=A(&iBI,iHI,iII,jLI,iFI,iCI,iSI,iBL,iHL,iIL,jLL,iFL,iCL,iSL)[xt-tB+7*ytL](xV,xn,yV,zV,n);
 I(m<n,z=z(aL(n));A(&jBI,jHI,jII,jBL,jHL,jIL)[xt-tB+3*ytL](xV,xn,yV,zV,n))y-z?y(z):z)
...
S A1(qt,x(al(now())))
S A1(qE,P(!xtZ,et1(x))P(xN-2,el1(x))x=cL(x);Lij x(0);P(i>j,ed1(x))aE(i,j))
A2(_1,/*01*/P(!xtt,i1(x,y))S O C s[][4]={"","k","j","p","t","x","hex","err","argv","env","exit","js","pri","prng","sin","cos","exp","ln","E"};
 Xs(Li=fI(s,L(s),xv);A(&ser,kst,js1,qp,qt,frk,hex,err,qa,qe,qx,qjs,qpri,prng,ksin,kcos,kexp,klog,qE,ed1)[i<0?L(s):i](y))
...
AX(_8,/*01..1*/Q(n)P(n==1,x1(*a))P(n==2&&!xtp,Ay=*a;y(x2(y,a[1])))P(!xtt,i8(x,a,n))Ik=xK;P(n<k,prj(x,a,n))Xo(run(x,a,n))Xv(x=v8[xv](a,n);mrn(n-1,a+1);x)Xr(w8(x,a,n))Xq(_1(xx,N(_8(xy,a,n))))
