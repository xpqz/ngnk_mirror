...
S A1(qt,x(al(now())))
A2(_1,/*01*/P(!xtt,i1(x,y))S O C s[][4]={"","k","j","p","t","x","hex","err","argv","env","exit","js","pri","prng","sin","cos","exp","ln"};
 Xs(Li=fI(s,L(s),xv);A(&ser,kst,js1,qp,qt,frk,hex,err,qa,qe,qx,qjs,qpri,prng,ksin,kcos,kexp,klog,ed1)[i<0?L(s):i](y))
...
A1(val,XS(P(!xn,x(0);Ax=oS,y=oA;i(gn,I(gv[i],xq(as(gk[i]));yq(_R(gv[i]))))am(x,y))val(jS(x)))Xs(Li=fI(gk,gn,xv);i<0||!gv[i]?ev0():_R(gv[i]))
 XC(x=str0(x);x(evC(xV,0)))Xc(val(enl(x)))XmM(x(_R(xy)))
