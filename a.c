...
S A1(sam,x)V_;T_;I _K(A x/*0*/)_(X(R2(tu,tw,1)Rv(2)Rx(x>>48&7)R4(to,tp,tq,tr,xk))0)
...
S A iM(A x,L i,FN(A,A,L)*f)_(Q(xtM);A y=xy,z=aA(yn);Q(ytA);j(zn|!zn,zA[j]=f(yA[j],i))am(_R(xx),sqz(z)))
...
A2(_1,/*01*/P(!xtt,i1(x,y))I k=xK;P(1<k,k==2&&!xtp?prj(x,A(y,GAP),2):prj(x,&y,1))
 X(Ro(run(x,&y,1))Rr(w1(xE,xx,y))Rx(((A1*)(x<<16>>16))(y))Rq(_1(xx,N(_1(xy,y))))Ru(v1[xv](y))Rw(AK(0<xv&&xv<4&&yK==2?1:ytU?yK:1,AW(xv,aV(tr,1,&y))))R_(et1(y))
  Rp(P(xn+k>9,er1(y))I m=xn-1,j=0;Ab8;i(m,b[i]=xA[i+1]==GAP&&!j?j++,y:_R(xA[i+1]))I l=MAX(0,1-j);MC(b+m,&y,8*l);_8(xx,b,m+l))
  Rs(S O C s[][4]={"k","j","p","t","x","hex","err","argv","env","exit","js","pri","prng","sin","cos","exp","ln","fb"};
   L i=fI(s,L(s),xv);G(&kst,js1,qp,qt,frk,hex,err,qa,qe,qx,qjs,qpri,prng,ksin,kcos,kexp,klog,qfb,ed1)[i<0?L(s):i](y)))0)
A3(_2,/*001*/P(!xtt,i2(x,y,z))A a[]={y,z};I k=xK;P(2<k,yR;prj(x,a,2))
 X(Rv(v2[xv](y,z))Ro(yR;run(x,a,2))Rr(z(w2(xE,xx,yR,z)))Rw(xv?en1(x):AK(zK,AT(tq,aA2(yR,z))))Rq(_1(xx,N(_2(xy,y,z))))Rx(P(2>k,er1(z))((A2*)(x<<16>>16))(yR,z))R_(et1(z))
  Rp(P(xn+k>9,er1(z))yR;I m=xn-1,j=0;Ab8;i(m,b[i]=xA[i+1]==GAP&&j<2?a[j++]:_R(xA[i+1]))I l=MAX(0,2-j);MC(b+m,a+j,8*l);_8(xx,b,m+l)))0)
AX(_8,/*01..1*/Q(n)P(n==1,x1(*a))P(n==2&&!xtp,A y=*a;y(x2(y,a[1])))P(!xtt,i8(x,a,n))P(n<xK,prj(x,a,n))
...
X1(val,RA(P(!xn,x)P(xn==1,fir(x))P(xn>9,ez1(x))x=mut(x);A y=_8(xx,&xy,xn-1);AN(1,x);x(y))RE(gZ(x))RZ_E(x)RC(x=str0(x);x(evs(xV,0)))Rc(val(enl(x)))Rs(val(enl(x)))
 RS(P(!xn,x(0);A x=oS,y=oA;i(gn,I(gv[i],L k=gk[i];xq(k-(U)k?jS(aV(tS,2,A((I)(k>>32),k))):as(k));yq(_R(gv[i]))))am(x,y))L i=fL(gk,gn,gkk(x));i<0||!gv[i]?ev0():_R(gv[i]))
...
S A set(A x,L i,A y/*1i1*/)_(Q(xr==1);XA(A z=xa;xa=z(y);ytt&&!ytU?sqz(x):x)XM(A z=kv(&x);z=mut(z);Q(ztA);I(ytT&&yN-zn,x(el2(y,z)))I j=i;i(zn,za=set(mut(za),j,ii(y,i));P(!za,za=au;x(y(z(0)))))y(aM(x,z)))
...
S A3(d3,/*100*/N m=yN;P(y==au||!m,z1(x))P(m==1,y=fir(yR);y(a3(x,y,z)))A u=prj(DOT,(A[]){GAP,drp(1,yR)},2);y=fir(yR);y(u(a4(x,y,u,z))))
A4(d4,/*1000*/N m=yN;P(y==au||!m,x(z2(x,uR)))P(m==1,y=fir(yR);y(a4(x,y,z,u)))A v=prj(DOT,(A[]){GAP,drp(1,yR)},2);y=fir(yR);A r=y(a5(x,y,v,z,u));mr(v);r)
S AA(d8_,/*10..0*/A x=*a,y=a[1],z=a[2];P(n==4,d4(x,y,z,a[3]))P(n==3,d3(x,y,z))en1(x))
AA(d8,/*10..0*/A x=*a;XsS(L k=gkk(x);x(0);L i=fL(gk,gn,k);I(i<0,gk[i=gn++]=k)A*p=gv+i;I(!*p,*p=au)Ab8;*b=*p;MC(b+1,a+1,n-1<<3);*p=au;*p=_R(N(d8_(b,n))))I(n==3,XU(try(x,a[1],a[2])))d8_(a,n))
