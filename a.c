...
S A1(qjs,XC(Cb[4096];In=js_eval(xC,xn,b,SZ b);x(0);aCn(b,n))XA(e1f(qjs,x))et1(x))
S A1(qp,XC(x=str0(x);Qs=xC;x(pk(&s)))et1(x))
S A1(qt,x(al(now())))
...
A1(val,
 SW(xt,
  R(tS,P(!xn,x(0);Ax=oS,y=oA;i(gn,I(gv[i],xq(as(gk[i]));yq(_R(gv[i]))))am(x,y))val(jS(x)))
  R(ts,Li=fI(gk,gn,xv);i<0||!gv[i]?ev0():_R(gv[i]))
  R(tC,x=str0(x);x(evC(xV,0)))
  R(tc,val(enl(x)))
  R2(tm,tM,x(_R(xy)))
  R(tA,P(!xn,x)P(xn==1,fir(x))P(xn>9,ez1(x))x=mut(x);Ay=_8(xx,&xy,xn-1);AN(1,x);x(y))
  R(tr,cat10(AT(tA,mut(x)),aw+xE))
  R3(tu,tv,tw,ai(xv))
  R3(to,tp,tq,AT(tA,mut(x)))
  R(tE,gZ(x)))
 et1(x))
