...
S A1(qt,x(al(now())))
S X1(qE,Rz(Ln=gl(x);aE(MIN(0,n),MAX(0,n)))R_(et1(x)))
...
X1(val,RA(P(!xn,x)P(xn==1,fir(x))P(xn>9,ez1(x))x=mut(x);Ay=_8(xx,&xy,xn-1);AN(1,x);x(y))RE(gZ(x))RZ_(x)RC(x=str0(x);x(evC(xV,0)))Rc(val(enl(x)))
