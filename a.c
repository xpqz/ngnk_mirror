...
Y2(iex,/*01*/RmMA(r2f(iex,x,y))RT_A(rsz(yN,iex(x,fir(y))))Rc(ac("\0\1\x7f\x80 "[iei(x)]))Rf(y(af(A(0.,1.,WF,-WF,NF)[iei(x)])))R_(y(az(A(0ll,1,WL,-WL,NL)[iei(x)]))))
...
AAL(ii,/*0*/X(RA(_R(xa))RC(ac(xc))RB(ai(xb))RH(ai(xh))RI(ai(xi))RL(al(xl))RF(af(xf))RS(as(xi))Rm(ii(xy,i))RM(iM(x,i,ii))RE(az(*xL+i))R_(xR))0)
...
#define iT(f,T,U,R,e...) S L f(OV*RES a,Lm,OV*RES b,V*RES c,Nn)_(O T*p=a;O U*q=b;R*r=c;LI(p)LI(q)LI(r)e;n)
#define j_(T,U)iT(j##T##U,T,U,L,i(n,r[i]=IN(q[i],m)?p[q[i]]:NL))
#define i_(T,U)iT(i##T##U,T,U,T,UI g=G(T,U);U k[g];i(n/g,j(g,k[j]=q[j])U o=0;j(g,o|=!IN(k[j],m))P(o,i*g)j(g,r[j]=p[k[j]])q+=g;r+=g)L l=n/g*g;j(n-l,P(!IN(q[j],m),l))j(n-l,r[j]=p[q[j]]))
iT(iCI,C,I,C,i(PD(n,q),r[i]=IN(q[i],m)?p[q[i]]:32))
iT(iCL,C,L,C,i(PD(n,q),r[i]=IN(q[i],m)?p[q[i]]:32))
iT(iFI,F,I,F,i(PD(n,r),r[i]=IN(q[i],m)?p[q[i]]:NF))
iT(iFL,F,L,F,i(PD(n,r),r[i]=IN(q[i],m)?p[q[i]]:NF))
iT(iSI,I,I,I,i(PD(n,q),r[i]=IN(q[i],m)?p[q[i]]:0 ))
iT(iSL,I,L,I,i(PD(n,q),r[i]=IN(q[i],m)?p[q[i]]:0 ))
i_(B,I)j_(B,I)i_(B,L)j_(B,L)i_(H,I)j_(H,I)i_(H,L)j_(H,L)i_(I,I)j_(I,I)i_(I,L)j_(I,L)j_(L,I)j_(L,L)
A2(i1,/*01*/Xt(y(xR))P(y==PLH||y==au,xR)P(xtm||xtM&&ytsS,i1(xy,N(fnd(xx,y))))
 Y(RA(r2(AP1,x,y))RmM(Az=kv(&y);am(y,Ny(i1(x,z))))R3(tB,tH,tC,i1(x,cI(y)))Rzc(io(x,gl(y)))R_(et1(y))RE(Li=*yL,j=yL[1];P(0<=i&&i<j&&j<xN,y(0);slc(x,i,j))i1(x,gZ(y)))
  R2(tI,tL,P(xtM,aM(_R(xx),N(l2f(i1,xy,y))))P(xtA,Nn=yn;Az=aA(n);n|=!n;I(ytL,i(n,za=io(x,yl)))E(i(n,za=io(x,yi)))y(0);I(!zn,zx=mkn(zx))sqz(z))
   Nn=yn;Az=xtL&&ytL&&yr==1?y:an(n,xt);Lm=A(&iBI,iHI,iII,jLI,iFI,iCI,iSI,iBL,iHL,iIL,jLL,iFL,iCL,iSL)[xt-tB+7*ytL](xV,xn,yV,zV,n);
   I(m<n,z=z(aL(n));A(&jBI,jHI,jII,jBL,jHL,jIL)[xt-tB+3*ytL](xV,xn,yV,zV,n))y-z?y(z):z))0)
...
S X1(qp,RC(x=str0(x);Qs=xC;x(pk(&s)))R_(et1(x)))
S A1(qt,x(al(now())))
S X1(qE,Rz(Ln=gl(x);n<0?aE(n,0):aE(0,n))R_(et1(x)))
A2(_1,/*01*/P(!xtt,i1(x,y))Ik=xK;P(1<k,k==2&&!xtp?prj(x,A(y,PLH),2):prj(x,&y,1))
 X(Ro(run(x,&y,1))Rr(w1(x,y))Rx(((A1*)(x<<16>>16))(y))Rq(_1(xx,N(_1(xy,y))))Ru(v1[xv](y))Rw(AK(0<xv&&xv<4&&yK==2?1:ytU?yK:1,AW(xv,aV(tr,1,&y))))R_(et1(y))
  Rp(P(xn+k>9,er1(y))Im=xn-1,j=0;Ab8;i(m,b[i]=xA[i+1]==PLH&&!j?j++,y:_R(xA[i+1]))I l=MAX(0,1-j);MC(b+m,&y,8*l);_8(xx,b,m+l))
  Rs(S O C s[][4]={"","k","j","p","t","x","hex","err","argv","env","exit","js","pri","prng","sin","cos","exp","ln","E"};
   Li=fI(s,L(s),xv);A(&ser,kst,js1,qp,qt,frk,hex,err,qa,qe,qx,qjs,qpri,prng,ksin,kcos,kexp,klog,qE,ed1)[i<0?L(s):i](y)))0)
A3(_2,/*001*/P(!xtt,i2(x,y,z))A a[]={y,z};Ik=xK;P(2<k,yR;prj(x,a,2))
 X(Rv(v2[xv](y,z))Ro(yR;run(x,a,2))Rr(z(w2(x,yR,z)))Rw(xv?en1(x):AK(zK,AT(tq,aA2(yR,z))))Rq(_1(xx,N(_2(xy,y,z))))Rx(P(2>k,er1(z))((A2*)(x<<16>>16))(yR,z))R_(et1(z))
...
AX(_8,/*01..1*/Q(n)P(n==1,x1(*a))P(n==2&&!xtp,Ay=*a;y(x2(y,a[1])))P(!xtt,i8(x,a,n))P(n<xK,prj(x,a,n))
 X(Ro(run(x,a,n))Rv(x=v8[xv](a,n);mrn(n-1,a+1);x)Rr(w8(x,a,n))Rq(_1(xx,N(_8(xy,a,n))))R_(et8(a,n))
...
X1(val,RA(P(!xn,x)P(xn==1,fir(x))P(xn>9,ez1(x))x=mut(x);Ay=_8(xx,&xy,xn-1);AN(1,x);x(y))RS(P(!xn,x(0);Ax=oS,y=oA;i(gn,I(gv[i],xq(as(gk[i]));yq(_R(gv[i]))))am(x,y))val(jS(x)))
 Rs(Li=fI(gk,gn,xv);i<0||!gv[i]?ev0():_R(gv[i]))RC(x=str0(x);x(evC(xV,0)))Rc(val(enl(x)))R(tE,gZ(x))Rr(cat10(AT(tA,mut(x)),aw+xE))RmM(x(_R(xy)))Ropq(AT(tA,mut(x)))
 Ruvw(ai(xv))R_(et1(x)))
