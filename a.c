...
i_(iB,B,B,UI g=32;I k[g];i(n/g,j(g,k[j]=q[j])I o=0;j(g,o|=!IN(k[j],m))P(o,i*g)j(g,r[j]=p[k[j]])q+=g;r+=g)L l=n/g*g;j(n-l,P(!IN(q[j],m),l))j(n-l,r[j]=p[q[j]]))
...
S L jB(V*RES a,Nm,OV*RES b,V*RES c,Nn)_(B*p=a;O I*q=b;L*r=c;LI(p)LI(q)LI(r)i(n,r[i]=IN(q[i],m)?p[q[i]]:NL)n)
