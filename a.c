...
AAL(ii,/*0*/X(RA(_R(xa))RC(ac(xc))RB(ai(xb))RH(ai(xh))RI(ai(xi))RL(al(xl))RF(af(xf))RS(as(xi))Rm(ii(xy,i))RM(iM(x,i,ii))R_(xR))0)
AAL(io,/*0*/X(RT(IN(i,xn)?ii(x,i):xn?mkn(ii(x,0)):xtA?_R(xx):_R(cn[xt]))Rt(xR)Rm(io(xy,i))RM(iM(x,i,io)))0)A1(fir,x(io(x,0)))A1(las,x(io(x,xN-1)))
