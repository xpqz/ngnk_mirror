...
S I skt(U h,UH p)_(If=socket(AF_INET,SOCK_STREAM,0);P(f<0,eo0())I v=setsockopt(f,IPPROTO_TCP,TCP_NODELAY,(I[]){1},4);P(v<0,eo0())
...
S I o(Ax/*1*/,I fl)_(Xz(gl(x))Xs(L v=xv;P(!v,1)Qs=qs(&v);osf(s,fl))XC(x=str0(x);I v;Mx(v=osf(xV,fl));v)et1(x))
...
S A frS(If,Nn)_(C b[1024];Ax=oC;W(n,Ik=read(f,b,MIN(SZ b,n));P(k<0,eo1(x))n-=k;x=cts(x,b,k);P(k-SZ b,x))x)                                               // read stream (only length)
...
A2(v2c,P(!xts||!ytA,et1(y))P(yn-2,el1(y))P(!_ts(yx)||!_ti(yy),et1(y))L v=xv,w=_v(yx);Ik=_v(yy);y(0);P(!k||k>8u,ed0())                                    // x 2:y
