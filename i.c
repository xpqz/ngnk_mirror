...
AL(cls,close(n);au)
A1(u0c,spl(N(u1c(x))))
A1(u1c,Xz(frs(gl(x)))If=N(o(x,O_RDONLY));P(f<3,frs(f))Im=fmd(f);x=(S_ISDIR(m)?frd:S_ISREG(m)?frm:frs)(f);close(f);x)
A2(v0c,YA(v0c(x,N(jc(10,y))))YC(v1c(x,apc(y,10)))et1(y))
A2(v1c,P(!ytC,et1(y))Xz(If=gl_(x);ye((f<3||!S_ISREG(fmd(f))?fws:fwm)(f,yV,yn)))If=N(o(xR,O_RDWR|O_CREAT|O_TRUNC));Az=v1c(ai(f),y);f>2&&close(f);z)
