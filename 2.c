...
S A admfF(Ax,Ay,If/*01v*/)_(SW(xtT|ytT<<1,
 R(0,x=Ny(cF(xR));y=Nx(cF(y));F a=gf(x),b=gf(y);af(f==3?a*b:a+b))
 R(1,y(admfF(y,xR,f)))
 R(2,Az=yr-1?aF(yn):y;Nn=zn+3&~3;F a=*xF;I(f==3,i(n,zf= a*yf))E(i(n,zf= a+yf))y-z?y(z):z)
 R_(P(xn-yn,el1(y))Az=yr-1?aF(yn):y;Nn=zn+3&~3;I(f==3,i(n,zf=xf*yf))E(i(n,zf=xf+yf))y-z?y(z):z))0)
S A admx(Ax,Ay,If/*01v*/)_(SW(xtT|ytT<<1,
 R(0,Lm=gl_(x),n=gl(y);az(f==3?m*n:m+n))
 R(1,y(admx(y,xR,f)))
 R(2,I ox=0;Az;Nn=yn,i=0;L a=gl_(x);I w=MAX(tZ(a)-tG,yw);y=cT[tG+w](y);z=yr-1?an(n,yt):y;
  W(1,I(f==3,S4(w,{G*b=yG-i*(y!=z);W(i<n,Iv=a*b[i];B(v-(G)v)zg=v;i++)},
  ...
  I(ox,x(0))y-z?y(z):z)
 R_(P(xn-yn,el1(y))I ox=0;Az;Nn=yn,i=0;I w=MAX(xw,yw);P(xw-w,x=cT[tG+w](xR);x(admx(x,y,f)))y=cT[tG+w](y);z=yr-1?an(n,yt):y;
  W(1,I(f==3,S4(w,{G*a=xG-i;G*b=yG-i*(y!=z);W(i<n,Iv=a[i]*(I)b[i];B(v-(G)v)zg=v;i++)},
  ...
  I(ox,x(0))y-z?y(z):z))0)

S A dvdfF(Ax,Ay,If/*01f*/)_(SW(xtT|ytT<<1,
 R(0,Fv=*xF;af(v/gf(y)))
 R(1,Fv=gf(y);Az=aF(xn);i(xn,zf=xf/v)z)
 R(2,Fv=*xF;Az=yr==1?y:aF(yn);i(yn,zf=v/yf)y-z?y(z):z)
 R_(P(xn-yn,el1(y))Az=yr==1?y:aF(xn);i(xn,zf=xf/yf)y-z?y(z):z))0)
S A dvdx(Ax,Ay,If/*01f*/)_(x=Ny(cF(xR));x(dvdfF(x,cF(y),f)))

S A modx(Ax,Ay,If/*01f*/)_(
 Yzc(fir(N(mod(x,enl(y)))))
 Lm=gl_(x);P(!m,y)
...
S A mmmz(If,Lv,Ax)_(X(Rzc(Lm=-(f==7);az(m^MIN(m^v,m^gl(x))))R_(et1(x))
 RZC_E(Ct=tZ(v),u=tG+xw;I(u<t||u-xt,x=cT[t](x))E(t=u)Nn=xn;Ay=xr==1?x:an(n,t);C w=t-tG;n+=31>>w;Lm=-(f==7);v^=m;
  S4(w,i(n&~31,yg=m^MIN(v,m^xg)),i(n&~15,yh=m^MIN(v,m^xh)),i(n&~7,yi=m^MIN(v,m^xi)),i(n&~3,yl=m^MIN(v,m^xl)))x-y?x(y):y))0)
S A mmmZ(Ax,Ay/*01*/,If)_(Y(Rzc(mmmz(gl(y),xR,f))R_(et1(y))
  RZC_E(P(xn-yn,el1(y))C w=xw;P(w<yw,x=cT[tG+yw](xR);x(mmmZ(x,y,f)))y=cT[tG+w](y);Nn=yn;Az=yr==1?y:an(n,tG+w);n+=31>>w;Lm=-(f==7);
   S4(w,i(n&~31,zg=m^MIN(m^xg,m^yg)),i(n&~15,zh=m^MIN(m^xh,m^yh)),i(n&~7,zi=m^MIN(m^xi,m^yi)),i(n&~3,zl=m^MIN(m^xl,m^yl)))y-z?y(z):z))0)
S A mmmx(Ax,Ay/*01*/,If){SW(xtT|ytT<<1,
 R(0,Lv=gl_(x),w=gl(y);az(f==7?MAX(v,w):MIN(v,w)))
 R(1,mmmz(f,gl(y),xR))
 R(2,mmmz(f,gl_(x),y))
 R_(mmmZ(x,y,f)))}
S A mmmfF(Ax,Ay,If/*01f*/)_(x=of1(xR);y=of1(y);x(of0(Nx(mmmx(x,y,f)))))
...
S A cmpz(Lv,Ax/*1*/,If)_(I w=xw;P(tG+w<tZ(v),rsz(x(xn),ai(f==8?v<0:f==9?v>0:0)))Nn=xn;Ay=aG(n);Mx(A(&ltng,ltnh,ltni,ltnl,gtng,gtnh,gtni,gtnl,eqlg,eqlh,eqli,eqll)[f-8<<2|w](v,xV,yG,n));y)
S A cmpZ(Ax,Ay/*01*/,If)_(P(xn-yn,el1(y))I w=xw;P(w<yw,x=cT[tG+yw](xR);x(cmpZ(x,y,f)))I(yw<w,y=cT[tG+w](y))Nn=xn;Az=aG(n);
...
S A cmpx(Ax,Ay/*01*/,If){SW(xtT<<1|ytT,R(0,Lv=gl_(x),w=gl(y);ai(f==8?v<w:f==9?v>w:v==w))R(1,cmpz(gl_(x),y,f))R(2,cmpz(gl(y),xR,f==8?9:f==9?8:f))R_(cmpZ(x,y,f)))}
S A cmpfF(Ax,Ay,If/*01f*/)_(x=of1(xR);y=of1(y);x(cmpx(x,y,f)))

S C f;//0=dex,1=add,2=sub,3=mul,4=dvd,5=mod,6=mnm,7=mxm,8=ltn,9=gtn,10=eql
A2(ari,Ct=xt,u=yt;Lv=1<<t|1<<u;
 P(!(v&~(1<<tG|1<<tH|1<<tI|1<<tL|1<<tC|1<<ti|1<<tl|1<<tc)),A(&admx,0,admx,dvdx,modx,mmmx,mmmx,cmpx,cmpx,cmpx)[f-1](x,y,f))
 P(v&(1<<tm|1<<tM|1<<tA),e2f(f==1?add:ari,x,y))
 P(t==tE,x=gZ(xR);x(ari(x,y)))
 P(u==tE,ari(x,gZ(y)))
 P(v&(1<<tf|1<<tF),
  P(f==5,Ln=gl_(x);P(!n,y)P(n<0,en1(y))K2("{y-x*(-x)!_y}",x,y))
  P(t-tf&&t-tF,x=Ny(cF(xR));x(ari(x,y)))P(u-tf&&u-tF,ari(x,N(cF(y))))
  A(&admfF,0,admfF,dvdfF,0,mmmfF,mmmfF,cmpfF,cmpfF,cmpx)[f-1](x,y,f))
 I(f-8<3u,
  P(v&1<<tS,P(f==10&!(v&~(1<<ts|1<<tS)),cmpx(x,y,f))e2f(ari,x,y))
