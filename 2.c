...
S A admZZ(Ax,Ay,If)_(P(xn-yn,el1(y))I ox=0;Nn=yn,i=0;I w=MAX(xw,yw);P(xw-w,x=cT[tG+w](xR);x(admZZ(x,y,f)))y=cT[tG+w](y);Az=yr-1?an(n,yt):y;
 ...
 I(ox,x(0))y-z?y(z):z)
S A admzZ(L a,Ay,If)_(Nn=yn,i=0;I w=MAX(tZ(a)-tG,yw);y=cT[tG+w](y);Az=yr-1?an(n,yt):y;
...
S A modzZ(Lm,Ay,If)_(P(!m,y)
 P(m<0,m=-m;Az=an(yn,yt);S4(yw,i(zn,C v=yg;zg=v<0?-1-~v/m:v/m),i(zn,H v=yh;zh=v<0?-1-~v/m:v/m),i(zn,I v=yi;zi=v<0?-1-~v/m:v/m),i(zn,L v=yl;zl=v<0?-1-~v/m:v/m))y(z))
...
S A mmmzZ(L v,Ay,If)_(Ct=tZ(v),u=tG+yw;I(u<t||u-yt,y=cT[t](y))E(t=u)Nn=yn;Az=yr-1?an(n,t):y;C w=t-tG;n+=31>>w;Lm=-(f==7);v^=m;
...
SN V ltng(L v,O V*RES a,V*RES b,Nn){G w=v;O G32*p=a;G32*r=b;i(n+31>>5,j(32,r[i][j]=w< p[i][j]))}
SN V ltnh(L v,O V*RES a,V*RES b,Nn){H w=v;O H16*p=a;G16*r=b;i(n+15>>4,j(16,r[i][j]=w< p[i][j]))}
SN V ltni(L v,O V*RES a,V*RES b,Nn){I w=v;O I8 *p=a;G8 *r=b;i(n+ 7>>3,j( 8,r[i][j]=w< p[i][j]))}
SN V ltnl(L v,O V*RES a,V*RES b,Nn){L w=v;O L4 *p=a;G4 *r=b;i(n+ 3>>2,j( 4,r[i][j]=w< p[i][j]))}
SN V gtng(L v,O V*RES a,V*RES b,Nn){G w=v;O G32*p=a;G32*r=b;i(n+31>>5,j(32,r[i][j]=w> p[i][j]))}
SN V gtnh(L v,O V*RES a,V*RES b,Nn){H w=v;O H16*p=a;G16*r=b;i(n+15>>4,j(16,r[i][j]=w> p[i][j]))}
SN V gtni(L v,O V*RES a,V*RES b,Nn){I w=v;O I8 *p=a;G8 *r=b;i(n+ 7>>3,j( 8,r[i][j]=w> p[i][j]))}
SN V gtnl(L v,O V*RES a,V*RES b,Nn){L w=v;O L4 *p=a;G4 *r=b;i(n+ 3>>2,j( 4,r[i][j]=w> p[i][j]))}
SN V eqlg(L v,O V*RES a,V*RES b,Nn){G w=v;O G32*p=a;G32*r=b;i(n+31>>5,j(32,r[i][j]=w==p[i][j]))}
SN V eqlh(L v,O V*RES a,V*RES b,Nn){H w=v;O H16*p=a;G16*r=b;i(n+15>>4,j(16,r[i][j]=w==p[i][j]))}
SN V eqli(L v,O V*RES a,V*RES b,Nn){I w=v;O I8 *p=a;G8 *r=b;i(n+ 7>>3,j( 8,r[i][j]=w==p[i][j]))}
SN V eqll(L v,O V*RES a,V*RES b,Nn){L w=v;O L4 *p=a;G4 *r=b;i(n+ 3>>2,j( 4,r[i][j]=w==p[i][j]))}
...
S A cmpzZ(L v,Ay,If)_(I w=yw;P(tG+w<tZ(v),y(rsz(yn,ai(f==8?v<0:f==9?v>0:0))))
...
S A admfF(F v,A y,I f)_(Az=yr-1?aF(yn):y;Nn=zn+3&~3;I(f==3,i(n,zf=v*yf))E(i(n,zf=v+yf))y-z?y(z):z)
S A dvdfF(F v,A y,I f)_(Az=yr-1?aF(yn):y;i(yn,zf=v/yf)y-z?y(z):z)
S A dvdFf(A x,F v,I f)_(Az=aF(xn);i(xn,zf=xf/v)z)
