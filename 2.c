...
S A3(mxms,/*001*/P((!y||ytz)&&ztZ,Lv=y?gl_(y):-WL,l=-1ll<<8*zW-1,h=~l;Nn=zn;I(v<=l||h<=v,P(v>=0,z(rsh(n,az(v))))v=v<0?l:h)
...
S A3(mxmp,/*001*/ZC(mxmp(x,y,cB(z)))I w=zw;Lv=gl_(y),l=-1ll<<(8<<w)-1,h=~l;MAX(v,l);Nn=zn;P(v>=h,rsh(n,z(az(v))))y=zr>1?zr--,an(zt,zn):z;
