...
Y2(mod,RmMA(e2f(mod,x,y))Rzc(fir(N(mod(x,enl(y)))))RC(mod(x,cG(y)))RE(mod(x,gZ(y)))RfF(Ln=gl_(x);P(!n,y)P(n<0,en1(y))K2("{y-x*(-x)!`i$y}",x,y))R_(ed1(y))
 RZ_E(Lm=gl_(x);P(!m,y)
...
S ALA(ext/*n1*/,X(RMT(x)R_(rsz(n,enl(x))))0)
A2(dct,P(rnk(x)<0,ed1(y))x=ext(yN,xR);y=ext(xN,y);(xN-yN?el2:am)(x,y))
...
S A mmm(Ax,Ay,A2 f)_(XYmMA(e2f(f,x,y))YE(mmm(x,gZ(y),f))XE(y(mmm(y,xR,f)))P(xtfF||ytfF,x=of1(Ny(cF(xR)));y=of1(Nx(cF(y)));of0(N(x(f(x,y)))))
 ...
 I(k==1,Lv=gl_(x);I(f==mnm,S4(zw,i(n,zg=MIN( v,yg)),i(n,zh=MIN( v,yh)),i(n,zi=MIN( v,yi)),i(n,zl=MIN( v,yl))))
                  E(       S4(zw,i(n,zg=MAX( v,yg)),i(n,zh=MAX( v,yh)),i(n,zi=MAX( v,yi)),i(n,zl=MAX( v,yl)))))
 E(Q(k==3);       I(f==mnm,S4(zw,i(n,zg=MIN(xg,yg)),i(n,zh=MIN(xh,yh)),i(n,zi=MIN(xi,yi)),i(n,zl=MIN(xl,yl))))
                  E(       S4(zw,i(n,zg=MAX(xg,yg)),i(n,zh=MAX(xh,yh)),i(n,zi=MAX(xi,yi)),i(n,zl=MAX(xl,yl)))))x(y-z?y(z):z))
A2(ltn,P((1<<xt|1<<yt)&(1<<tm|1<<tM|1<<tA|1<<tS),e2f(ltn,x,y))YE(ltn(x,gZ(y)))XE(x=gZ(xR);x(ltn(x,y)))
 ...
 P(k==3&&xn-yn,el1(y))xR;N(sup(&x,&y));XfF(x=of1(x);x(ltn(x,of1(y))))P(k==2,y=az(gl(y)-1);not(N(y(ltn(y,x)))))Nn=k-1?xn:yn;Az=aG(n);
 I(k==1,Lv=gl_(x);S4(yw,i(n,zg=v<yg),i(n,zg=v<yh),i(n,zg=v<yi),i(n,zg=v<yl)))E(Q(k==3);S4(xw,i(n,zg=xg<yg),i(n,zg=xh<yh),i(n,zg=xi<yi),i(n,zg=xl<yl)))x(y(z)))
A2(eql,XYmMA(e2f(eql,x,y))P(xtsS-ytsS,et1(y))P(xtsS,x=AT(xt+ti-ts,mut(xR));x(eql(x,AT(yt+ti-ts,mut(y)))))
 Ik=xtT<<1|ytT;P(!k,P(xtzc&&ytzc,ai(gl_(x)==gl(y)))P(xtf||ytf,x=Ny(cF(xR));y=Nx(cF(y));ai(gl(x)==gl(y)))et1(y))
 YE(Xz(Li=*yL,n=yL[1]-i,v=gl_(x)-i;y(0);x=aG(n);MS(xV,0,n);I(IN(v,n),xG[v]=1)x)eql(x,gZ(y)))XE(y(eql(y,xR)))P(k==2,y(eql(y,xR)))P(k==3&&xn-yn,el1(y))
 xR;N(sup(&x,&y));Ct=TT[k?MIN(xt,yt):MAX(xt,yt)];I w=Tw[t];Nn=k-1?xn:yn;Az=aG(n);
 I(k==1,S4(w,{Cv=xv;i(n,zg=v==yg)},{Hv=xv;i(n,zg=v==yh)},{Iv=xv;i(n,zg=v==yi)},{Lv=gl_(x);i(n,zg=v==yl)}))
 E(Q(k==3);S4(w,i(n,zg=xg==yg),i(n,zg=xh==yh),i(n,zg=xi==yi),i(n,zg=xl==yl)))x(y(z)))
