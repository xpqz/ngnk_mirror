...
S A3(mmmf,Ii=zv==7;V(f,i?mxmfZ:mnmfZ)
 P((x&&xtd)||ytD,Lv=ordd(x?gd(cD(x)):i?-WD:WD);y=cD(y);ye(OFP(y);L r=f(v,y);OFP(y);ad(dord(r))))
...
S A3(admf,Q(ztv)I(ytC,y=cB(y))I(ytD||(x&&xtd),I(x,x=cD(x))y=cD(y))
 ...
 V(f,T(&addfB,addfH,addfI,addfL,mulfB,mulfH,mulfI,mulfL)[i<<2|yt-tB])az(ye(f(x?gl(x):T(0ll,1,WL,-WL)[i],yV,yn))))
