...
S A admx(Ax,Ay,If/*01v*/){SW(xtT|ytT<<1,
...
S A dvdx(Ax,Ay,If/*01f*/)_(x=Ny(cF(xR));x(dvdf(x,cF(y),f)))
...
S A mmmz(If,Lv,Ax){X(Rzc(Lm=-(f==7);az(m^MIN(m^v,m^gl(x))))R_(et1(x))
...
S A mmmZ(Ax,Ay,If/*01f*/){Y(Rzc(mmmz(gl(y),xR,f))R_(et1(y))
...
S A mmmx(Ax,Ay,If/*01f*/){SW(xtT|ytT<<1,
 R(0,Lv=gl_(x),w=gl(y);az(f==7?MAX(v,w):MIN(v,w)))
 R(1,mmmz(f,gl(y),xR))
 R(2,mmmz(f,gl_(x),y))
 R_(mmmZ(x,y,f)))}
S A mmmf(Ax,Ay,If/*01f*/)_(x=of1(xR);y=of1(y);x(of0(Nx(mmmx(x,y,f)))))
...
S A cmpz(Lv,Ax,If/*v1f*/)_(I w=xw;P(tG+w<tZ(v),rsz(x(xn),ai(f==8?v<0:f==9?v>0:0)))Nn=xn;Ay=aG(n);Mx(A(&ltng,ltnh,ltni,ltnl,gtng,gtnh,gtni,gtnl,eqlg,eqlh,eqli,eqll)[f-8<<2|w](v,xV,yG,n));y)
S A cmpZ(Ax,Ay,If/*01f*/)_(P(xn-yn,el1(y))I w=xw;P(w<yw,x=cT[tG+yw](xR);x(cmpZ(x,y,f)))I(yw<w,y=cT[tG+w](y))Nn=xn;Az=aG(n);
...
S A cmpx(Ax,Ay,If/*01f*/){SW(xtT<<1|ytT,R(0,Lv=gl_(x),w=gl(y);ai(f==8?v<w:f==9?v>w:v==w))R(1,cmpz(gl_(x),y,f))R(2,cmpz(gl(y),xR,f==8?9:f==9?8:f))R_(cmpZ(x,y,f)))}
S A cmpf(Ax,Ay,If/*01f*/)_(x=of1(xR);y=of1(y);x(cmpx(x,y,f)))
...
A2(ari,Ct=xt,u=yt;Lv=1<<t|1<<u;
 ...
 P(v&(1<<tf|1<<tF),
  ...
  A(&admf,0,admf,dvdf,0,mmmf,mmmf,cmpf,cmpf,cmpx)[f-1](x,y,f))
 I(f-8<3u,
