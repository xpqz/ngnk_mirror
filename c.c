...
X1(cB,RmMA(e1f(cB,x))             RB(x) Rilc(ai(gl(x)&1))RG(tfx(tB,cBG,x))                                                        R_(en(x)))
X1(cC,RmMA(e1f(cC,x))RE(cC(gZ(x)))RcC(x)Ril(ac(gl(x)))   RG(AT(tC,mut(x)))                                                        R_(cC(N(cG(x)))))
...
X1(cI,RmMA(e1f(cI,x))RE(cI(gZ(x)))RI(x) Rilc(ai(gl(x)))  RfF(cI(N(cL(x))))R4(tG,tH,tL,tC,tfx(tI,G(&cIG,cIH,0,cIL,0,cIG)[xt-tG],x))R_(et(x)))
X1(cL,RmMA(e1f(cL,x))RE(cL(gZ(x)))RlL(x)Ric(al(xv))      RF(tfx(tL,cLF,x))Rf(al(gf(x)))                                           R_(tfx(tL,cLI,N(cI(x)))))
X1(cF,RmMA(e1f(cF,x))             RfF(x)Rilc(af(gl(x)))                                                                           R_(tfx(tF,cFL,N(cL(x)))))
X1(cS,RmMA(e1f(cS,x))             RsS(x)RC(x=str0(x);x(sym(xV)))Rc(as(xv))                                                        R_(et(x)))
A1*cT[]={[tB]=cB,[tG]=cG,cH,cI,cL,[tF]=cF,[tC]=cC,[tS]=cS};
S X1(pI,RmMA(e1f(pI,x))Rc(pI(enl(x)))RC(x=str0(x);Q s=xV;P(!*s,x(_R(cn[tl])))L v=pl(&s);x(*s?_R(cn[tl]):az(v)      ))R_(et(x)))
S X1(pF,RmMA(e1f(pF,x))Rc(pF(enl(x)))RC(x=str0(x);Q s=xV;P(!*s,x(_R(cn[tf])))L v=pf(&s);x(*s?_R(cn[tf]):aV(tf,1,&v)))R_(et(x)))
S X1(csti,RmMA(e1f(csti,x))RF(sqzZ(cL(x)))Rf(az(gf(x)))RC(cG(x))Rc(ai(xv))Ruvw(ai(xv))RilEGHIL(x)R_(et(x)))
S Y2(pad,RmMA(e2f(pad,x,y))RC(K2("{y@(!x)+(x<0)*#y}",x,y))Rc(cst(x,enl(y)))R_(et(y)))
X2(cst,Rs(I v=xv;P(v-(C)v,ed(y))G(&csti,cF,cC,cS,pI,pF,ed)[si("ifcsIF",v|'s'*!v)](y))Ril(pad(x,y))R_(et(y)))
