...
ALA(tak,/*1*/Xt(tak(n,enl(x)))XM(Ay=kv(&x),z=az(n);aM(x,Nx(z(r2(RSH,z,y)))))Xm(Ay=kv(&x);x=Ny(tak(n,x));y=Nx(tak(n,y));am(x,y))Q(xtT)P(n==NL,x)P(!xn,tak(n,enl(fir(x))))
 I r=n<0;n*=1-2*r;Ay=an(xt,n);N w=xW,m=xn*w,k=n%xn*w,l=n*w;I(!r,Mc(yV,xV,min(m,l)))J(l<=m,Mc(yV,xV+m-l,l))E(Mc(yV,xV+m-k,k);Mc(yV+k,xV,m-k))cyc(yV,m,l);x(ytA?sqz(mRa(y)):y))
...
S A2(rsh,/*01*/YmM(en1(y))Yt(rsh(x,enl(y)))Q(xtZ);N r=xn;P(!r,fir(y))P(r>256,ez1(y))x=Ny(cL(xR));L s[r];Mc(s,xV,r<<3);x(0);
 ...
 L p[r],m=1;i(r,L d=s[r-1-i];P(d<0||m*d<0,ed1(y))p[i]=m*=max(1,d))y=N(tak(m,y));i(r-1,L d=s[r-1-i];I(d,y=N(chp(max(1,d),y)))E(y=N(e1f(cy0,chp(1,y)))))tak(*s,y))
A2(hsh,/*01*/Xz(tak(gl_(x),y))XZ(rsh(x,y))XF(flt(x,y,1))XT(P(ytm||ytM&&xtS,a2t(xR,y(y1(xR)),yt))et1(y))et1(y))
