...
//__builtin_ia32_ldmxcsr(__builtin_ia32_stmxcsr()|1<<6|1<<15); //daz,ftz
 ...
 I open(Qp,I v,...)_(Im=SL(p);P(m>=SZ s[0].p,ENAMETOOLONG)Ii=0;W(i<ns&&strcmp(s[i].p,p),i++)
 ...
 I dup2(If,I v)_(-1)I execve(Qp,C*O*a,C*O*e)_(-1)I fork()_(-1)I socket(Ii,Ij,Ik)_(-1)
 ...
 V*memset(V*x,I v,Nn)_(C*p=x;i(n,*p++=v);x)
 V*memchr(OV*x,I v,Nn)_(Qs=x;i(n,P(s[i]==v,(V*)(s+i)))(V*)0)
 V*memmem(OV*x,Nm,OV*y,Nn)_(Qp=x,q=y;i((L)m-(L)n+1,P(!memcmp(p+i,q,n),(V*)(p+i)))(V*)0)
 I memcmp(OV*x,OV*y,Nn)_(Qs=x,t=y;i(n,I v=*s++-*t++;P(v,v))0)
 ...
 C*strchrnul(Qs,I v)_(C*p=strchr(s,v);p?p:(V*)s+SL(s))
#endif
