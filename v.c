...
AL(tilL,P(n<0,n==(I)n?add(ai(n),N(tilL(-n))):ed0())P(n>1<<30,ez0())Ax=an(n,tZ(n-1));tilV(xV,n,xt-tG);x)
X1(til,
// Rz(tilL(gl(x)))
...
X1(rev,Rm(Ay=kv(&x);am(rev(x),rev(y)))RM(Ay=kv(&x);aM(x,e1f(rev,y)))Rt(x)RE(rev(gZ(x)))
 R_(P(xn<2,x)x=mut(x);Ii=0,j=xn-1,w=xw;W(i<j,S4(w,SWP(xg,xG[j]),SWP(xh,xH[j]),SWP(xi,xI[j]),SWP(xl,xL[j]))i++;j--)x))
...
X1(unq,RM(en1(x))Rm(unq(val(x)))RE(x)RS(cSI(unq(cSI(x))))Rz(rndF(gl(x)))R_(et1(x))RGC(C a[256]={};Ay=an(0,xt);i(xn,UC c=xc;I(!a[c],a[c]=1;y=apc(y,xc)))x(y))
...
X1(cC,RcC(x)RG(AT(tC,mut(x)))Rz(ac(gl(x)))RE(cC(gZ(x)))R_(cC(N(cG(x)))))
X1(cG,RG(x)Rt(cI(x))RC(AT(tG,mut(x)))R_(x=N(cI(x));Ay=aG(xn);Mx(cIG(xV,yV,yn))y))
X1(cH,RH(x)Rt(cI(x))R_(x=N(cI(x));Ay=an(xn,tH);Mx(cIH(xV,yV,yn))y))
X1(cI,RI(x)Rzc(ai(gl(x)))RfF(cI(N(cL(x))))R4(tG,tH,tL,tC,Ay=aI(xn);Ii=xt-tG;Mx(A(&cGI,cHI,0,cLI,0,cGI)[i](xV,yV,yn))y)RE(gZ(x))R_(et1(x)))
...
X1(csti,RF(sqzZ(cL(x)))Rf(az(gf(x)))RC(cG(x))Rc(ai(xv))Ruvw(ai(xv))RzZ(x)R_(et1(x)))
