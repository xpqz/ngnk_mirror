...
X1(flp,Rt(enl(enl(x)))R_(enl(x))RM(Ay=kv(&x);am(x,y))
 ...
 RA(Nm=xn;Ln=cfm(xA,m|!m);P(n==-1,enl(x))P(n<0,el1(x))Ct=_t(xx);I(t<tM&&t-tE,i(m,Ay=xa;B(yt-t,t=0)))E(t=0)Ay=aA(n);
  ...
  x(0);I(!n,yx=mkn(yx))y))
V tilV(V*p,L v,L n,I w){L*a=p,k=G(0x101010101010101ll,0x1000100010001ll,1ll<<32|1,1)[w],d=k<<3-w;v*=k;v+=G(0x706050403020100ll,0x3000200010000ll,1ll<<32,0)[w];
 i((n-1>>3-w)+4&-4,a[i]=v;v+=d)}
A1(gZ,Lij x(0);P(i<0,x=az(i);x(add(x,gZ(aE(0,j-i)))))Ct=MAX(tZ(i),tZ(j-1));x=an(j-i,t);tilV(xV,i,j-i,t-tG);x)
X1(til,RA(K1("{x@'!#'x}",x))Rz(Ln=gl(x);n*=n!=-n;aE(MIN(0,n),MAX(0,n)))RZ(K1("{(*a)#'&'x#'1_a:|*\\|x,1}",x))RS(Ik=_v(jS(x));Ay=oS;i(gn,I(gk[i]>>32==k,yq(as(gk[i]))))y)
...
A1(typ,x(as(TS[xt])))
A1(len,x(az(xN)))
U _N(Ax/*0*/){X(RE(Lij j-i)RT_E(xn)Rm(_N(xy))RM(_N(_x(xy)))R_(1))}
...
X2(crt,Rt(fil(x,y))R_(en1(y))
 RT(I v=rnk(y);P(!v,crt(x,enl(y)))
  ...
  K2("{x@&~(!0),x~\\:y}/",x,y)))
A wdn(A x,U i,U j,U n)_(Ct=xt;Q(xtZC);Ay=an(n,t+1);Mx(G(&cGH,cHI,cIL,cLF,0,cGH)[t-tG](xV+TW[t]*i,yV,j-i))y)
...
X1(pI,RmMA(e1f(pI,x))Rc(pI(enl(x)))RC(x=str0(x);Qs=xV;P(!*s,x(_R(cn[tl])))L v=pl(&s);x(*s?_R(cn[tl]):az(v)      ))R_(et1(x)))
X1(pF,RmMA(e1f(pF,x))Rc(pF(enl(x)))RC(x=str0(x);Qs=xV;P(!*s,x(_R(cn[tf])))L v=pf(&s);x(*s?_R(cn[tf]):aV(tf,1,&v)))R_(et1(x)))
...
X1(imn,RF(imn(of1(x)))RE(Lij x(0);az(NL*(i==j)))R_(fir(N(asc(x))))
 RZC_E(Nn=xn;Lj=n?0:NL;S4(xw,{G v=*xG;i(n,I(xg<v,v=xg;j=i))},{H v=*xH;i(n,I(xh<v,v=xh;j=i))},{I v=*xI;i(n,I(xi<v,v=xi;j=i))},{L v=*xL;i(n,I(xl<v,v=xl;j=i))})x(az(j))))
