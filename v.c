...
A1(flp,XM(Ay=kv(&x);am(x,y))Xm(Ay=kv(&x);I(!ytA,y=flp(flp(y)))Ln=cfm(yA,yn);P(n<0,x(el1(y)))i(yn,Az=ya;I(ztt,y=mut(y);ya=rsz(n,z)))aM(x,y))Xt(enl(enl(x)))P(!xtA||!xn,enl(x))
 ...
 I w=Tw[t];j(n,Az=yA[j]=an(t,xn);S4(w,i(xn,zb=_B(xa)[j]),i(xn,zh=_H(xa)[j]),i(xn,zi=_I(xa)[j]),i(xn,zl=_L(xa)[j])I(w==4,yA[j]=sqz(mRa(z)))))x(y))
