...
#define c_(T,R,p) SN V c##T##R(OV*RES v,V*RES w,Nn)_(O T*RES a=v;R*RES r=w;i(((n)+HD/SZ(*p)-1&-HD/SZ(*p)),r[i]=a[i]))
...
X1(flp,Rt(enl(enl(x)))R_(enl(x))RM(Ay=kv(&x);am(x,y))
 ...
 RA(Nm=xn;Ln=cfm(xA,m|!m);P(n==-1,enl(x))P(n<0,el1(x))Ct=_t(xx);I(t<tM,i(m,Ay=xa;B(yt-t,t=0)))E(t=0)Ay=aA(n);
  ...
  x(0);I(!n,yx=mkn(yx))y))
V tilV(V*p,Lv,Ln,I w){L*a=p,k=A(0x101010101010101ll,0x1000100010001ll,1ll<<32|1,1)[w],d=k<<3-w;v*=k;v+=A(0x706050403020100ll,0x3000200010000ll,1ll<<32,0)[w];
 i((n-1>>3-w)+4&-4,a[i]=v;v+=d)}
A1(gZ,Lij x(0);P(i<0,x=az(i);x(add(x,gZ(aE(0,j-i)))))Ct=MAX(tZ(i),tZ(j-1));x=an(j-i,t);tilV(xV,i,j-i,t-tG);x)
X1(til,Rz(Ln=gl(x);aE(MIN(0,n),MAX(0,n)))RZ(K1("{(*a)#'&'x#'1_a:|*\\|x,1}",x))RmM(x(_R(xx)))Ro(val(x))RS(K1("{x:\".\"/$x,`;`$(#x)_'(x~/:@[;!#x]')#$!.0#`}",x))R_(et1(x)))
X1(whr,Rz(whr(enl(x)))RA(K1("{$[`A~@x;(,&#'*'x),,'/x@\\:!0|/#'x:o'x;,&x]}",x))Rm(Ay=kv(&x);x(x1(Nx(whr(y)))))RM(et1(x))RE(whr(gZ(x)))R_(et1(x))
...
A wdn(Ax,Ni,Nj,Nn)_(Ct=xt;Q(xtZC);Ay=an(n,t+1);Mx(A(&cGH,cHI,cIL,cLF,0,cGH)[t-tG](xV+TW[t]*i,yV,j-i))y)
...
X1(cI,RmMA(e1f(cI,x))RI(x)Rzc(ai(gl(x)))RfF(cI(N(cL(x))))R4(tG,tH,tL,tC,Ay=aI(xn);Ii=xt-tG;Mx(A(&cGI,cHI,0,cLI,0,cGI)[i](xV,yV,yn))y)RE(gZ(x))R_(et1(x)))
...
X2(cst,Rs(Iv=xv;P(v-(C)v,ed1(y))A(&csti,cF,cC,cS,pI,pF,ed1)[si("ifcsIF",v+'s'*!v)](y))RS(e2f(cst,x,y))Rz(cstz(x,y))R_(et1(y)))
...
A1(sqz,P(!xtA,x)Nn=xn;Ay=xx;Ct=yt;P(ytMT||ytU,x)
 ...
 y=an(n,TT[t]);Iv=yw;I(v<3,A(&cLG,cLH,cLI)[v](xV,yV,n))E(i(n,yl=gl_(xa)))x(y))
