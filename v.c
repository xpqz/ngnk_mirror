...
A1(rev,SW(xt,Rm(Ay=kv(&x);am(rev(x),rev(y)))RM(Ay=kv(&x);aM(x,e1f(rev,y)))RE(rev(gZ(x))))
 Xt(x)P(xn<2,x)x=mut(x);Ii=0,j=xn-1,w=xw;W(i<j,S4(w,SWP(xb,xB[j]),SWP(xh,xH[j]),SWP(xi,xI[j]),SWP(xl,xL[j]))i++;j--)x)
A1(typ,x(as(TS[xt])))A1(len,x(az(xN)))N _N(Ax/*0*/)_(XE(Lij j-i)XT(xn)Xm(_N(xy))XM(_N(_x(xy)))1)
S A1(cSI,Q(xtS||xtI)Ct=tS^tI^xt;xr-1?x(aV(t,xn,xV)):AT(t,x))
A1(unq,
 XE(x)
 XT(P(xn<2,x)P(xn<<xw<pg&&!xtA,K1("{x@&(x?x)=!#x}",x))P(xtB||xtC,C a[256]={};Ay=an(0,xt);i(xn,I(!a[(UI)xc],a[(UI)xc]=1;y=apc(y,xc)))x(y))XS(cSI(unq(cSI(x))))K1("{x@i@<i@:&@[;0;:;1]@~~':x@i:<x}",x))
 XM(en1(x))
 Xm(unq(val(x)))
 Xz(Ln=gl(x);P(n<0,ed0())rndF(n))
 et1(x))
...
A wdn(Ax,Ni,Nj,Nn)_(Ct=xt;Q(xtZC);Ay=an(n,t+1);xe(A(&cBH,cHI,cIL,cLF,0,cBH)[t-tB](xV+TW[t]*i,yV,j-i);y))
X1(cA,RA(x)Rt(aA1(x))Rm(et1(x))R_(Nn=xN;Ay=aA(n);i(n|!n,ya=ii(x,i))x(0);I(!n,yx=mkn(yx))y))
X1(cC,RC(x)RB(AT(tC,mut(x)))R_(x=N(cB(x));Xt(ac((C)xv))AT(tC,mut(x))))
X1(cB,RB(x)RC(AT(tB,mut(x)))R_(x=N(cI(x));Xt(x)Ay=an(xn,tB);xe(cIB(xV,yV,yn);y)))
X1(cH,RH(x)R_(x=N(cI(x));Xt(x)Ay=an(xn,tH);xe(cIH(xV,yV,yn);y)))
X1(cI,RI(x)Rzc(ai(gl(x)))RfF(cI(N(cL(x))))R4(tB,tH,tL,tC,Ay=aI(xn);Ii=xt-tB;xe(A(&cBI,cHI,0,cLI,0,cBI)[i](xV,yV,yn);y))RE(gZ(x))R_(et1(x)))
...
S A1(cstu,Xi(xv<22u?au+xv:ed0())et1(x))
S A1(cstv,Xi(xv<22u?av+xv:ed0())et1(x))
S A1(cstw,Xi(xv< 6u?aw+xv:ed0())et1(x))
A2(cst,/*01*/P(xtS||ytmMA,e2f(cst,x,y))Xz(YC(K2("{y@(!x)+(x<0)*#y}",x,y))Yc(cst(x,enl(y)))et1(y))
 Xs(Iv=xv;P(v-(C)v,ed1(y))A(&csti,cF,cC,cS,cstu,cstv,cstw,pI,pF,ed1)[si("ifcsuvwIF",v+'s'*!v)](y))et1(y))
...
X1(imn,RZC_(Nn=xn;Lj=n?0:NL;S4(xw,{Bv=*xB;i(n,I(xb<v,v=xb;j=i))},{Hv=*xH;i(n,I(xh<v,v=xh;j=i))},{Iv=*xI;i(n,I(xi<v,v=xi;j=i))},{Lv=*xL;i(n,I(xl<v,v=xl;j=i))})x(az(j)))
 RF(imn(of1(x)))RE(Lij x(0);az(NL*(i==j)))R_(fir(N(asc(x)))))
X1(imx,RZC_(imn(inv(x)))RF(imx(of1(x)))RE(Lij x(0);az(j-i?j-i-1:NL))R_(fir(N(dsc(x)))))
