...
SN V cHG(V*RES a,O V*RES b,U n)_(H*r=a;O G*p=b;i(n+15&-16,r[i]=p[i]))
SN V cIG(V*RES a,O V*RES b,U n)_(I*r=a;O G*p=b;i(n+ 7& -8,r[i]=p[i]))
SN V cGH(V*RES a,O V*RES b,U n)_(G*r=a;O H*p=b;i(n+31&-32,r[i]=p[i]))
SN V cIH(V*RES a,O V*RES b,U n)_(I*r=a;O H*p=b;i(n+ 7& -8,r[i]=p[i]))
SN V cLH(V*RES a,O V*RES b,U n)_(L*r=a;O H*p=b;i(n+ 3& -4,r[i]=p[i]))
SN V cGI(V*RES a,O V*RES b,U n)_(G*r=a;O I*p=b;i(n+ 7& -8,r[i]=p[i]))
SN V cHI(V*RES a,O V*RES b,U n)_(H*r=a;O I*p=b;i(n+ 7& -8,r[i]=p[i]))
SN V cLI(V*RES a,O V*RES b,U n)_(L*r=a;O I*p=b;i(n+ 3& -4,r[i]=p[i]))
SN V cGL(V*RES a,O V*RES b,U n)_(G*r=a;O L*p=b;i(n+ 3& -4,r[i]=p[i]))
SN V cHL(V*RES a,O V*RES b,U n)_(H*r=a;O L*p=b;i(n+ 3& -4,r[i]=p[i]))
SN V cIL(V*RES a,O V*RES b,U n)_(I*r=a;O L*p=b;i(n+ 3& -4,r[i]=p[i]))
...
X1(cB,RmMA(e1f(cB,x))Ril(ai(gl(x)&1))RG(A y=aB(xn);Mx(cBG(yV,xV,yn));y)R_(en1(x)))
X1(cC,RmMA(e1f(cC,x))RcC(x)RG(AT(tC,mut(x)))Ril(ac(gl(x)))RE(cC(gZ(x)))R_(cC(N(cG(x)))))
X1(cG,RmMA(e1f(cG,x))RG(x)Rt(cI(x))RC(AT(tG,mut(x)))RB(A y=aG(xn);Mx(cGB(yV,xV,yn))y)R_(x=N(cI(x));A y=aG(xn);Mx(cGI(yV,xV,yn))y))
X1(cH,RmMA(e1f(cH,x))RH(x)Rt(cI(x))R_(x=N(cI(x));A y=an(xn,tH);Mx(cHI(yV,xV,yn))y))
X1(cI,RmMA(e1f(cI,x))RI(x)Rilc(ai(gl(x)))RfF(cI(N(cL(x))))R4(tG,tH,tL,tC,A y=aI(xn);I i=xt-tG;Mx(G(&cIG,cIH,0,cIL,0,cIG)[i](yV,xV,yn))y)RE(gZ(x))R_(et1(x)))
X1(cL,RmMA(e1f(cL,x))RlL(x)Ric(al(xv))RF(A y=aL(xn);Mx(cLF(yV,xV,yn))y)Rf(al(gf(x)))RE(cL(gZ(x)))R_(x=N(cI(x));A y=aL(xn);Mx(cLI(yV,xV,yn))y))
X1(cF,RmMA(e1f(cF,x))RfF(x)Rilc(af(gl(x)))R_(x=N(cL(x));A y=aF(xn);Mx(cFL(yV,xV,yn))y))
...
A1*cT[]={[tA]=cA,[tE]=cL,[tB]=cB,[tG]=cG,cH,cI,cL,[tF]=cF,[tC]=cC,[tS]=cS};
X1(csti,RmMA(e1f(csti,x))RF(sqzZ(cL(x)))Rf(az(gf(x)))RC(cG(x))Rc(ai(xv))Ruvw(ai(xv))RilEGHIL(x)R_(et1(x)))
Y2(cstz,RmMA(e2f(cstz,x,y))RC(K2("{y@(!x)+(x<0)*#y}",x,y))Rc(cst(x,enl(y)))R_(et1(y)))
X2(cst,Rs(I v=xv;P(v-(C)v,ed1(y))G(&csti,cF,cC,cS,pI,pF,ed1)[si("ifcsIF",v+'s'*!v)](y))Ril(cstz(x,y))R_(et1(y)))
