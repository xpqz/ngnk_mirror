...
SN V cGB(O V*RES a,V*RES b,U n)_(O G*p=a;G*r=b;MS(r,0,n+7>>3);i(n,r[i>>3]|=(p[i]&1)<<(i&7)))
...
X2(crt,Rt(fil(x,y))R_(en1(y))
 RT(I v=rnk(y);P(!v,crt(x,enl(y)))
  ...
  K2("{x@&~(!0),x~\\:y}/",x,y)))
A wdn(A x,U i,U j,U n)_(C t=xt;Q(xtZC);A y=an(n,t+1);Mx(G(&cGH,cHI,cIL,cLF,0,cGH)[t-tG](xV+((W)i*TW[t]>>3),yV,j-i))y)
X1(cA,RA(x)Rt(aA1(x))Rm(et1(x))R_(U n=xN;A y=aA(n);i(n|!n,ya=ii(x,i))x(0);I(!n,yx=mkn(yx))y))
X1(cB,RmMA(e1f(cB,x))Ril(ai(gl(x)&1))RG(A y=aB(xn);Mx(cGB(xV,yV,yn));y)R_(en1(x)))
X1(cC,RmMA(e1f(cC,x))RcC(x)RG(AT(tC,mut(x)))Ril(ac(gl(x)))RE(cC(gZ(x)))R_(cC(N(cG(x)))))
X1(cG,RmMA(e1f(cG,x))RG(x)Rt(cI(x))RC(AT(tG,mut(x)))RB(A y=aG(xn);Mx(cBG(xV,yV,yn))y)R_(x=N(cI(x));A y=aG(xn);Mx(cIG(xV,yV,yn))y))
X1(cH,RmMA(e1f(cH,x))RH(x)Rt(cI(x))R_(x=N(cI(x));A y=an(xn,tH);Mx(cIH(xV,yV,yn))y))
X1(cI,RmMA(e1f(cI,x))RI(x)Rilc(ai(gl(x)))RfF(cI(N(cL(x))))R4(tG,tH,tL,tC,A y=aI(xn);I i=xt-tG;Mx(G(&cGI,cHI,0,cLI,0,cGI)[i](xV,yV,yn))y)RE(gZ(x))R_(et1(x)))
...
X1(cF,RmMA(e1f(cF,x))RfF(x)Rilc(af(gl(x)))R_(x=N(cL(x));A y=aF(xn);Mx(cLF(xV,yV,yn))y))
