...
A1(flp,XM(Ay=kv(&x);am(x,y))
 ...
 I w=Tz[t];i(n,Az=ya=an(t,xn);j(xn,Ay=xA[j];I(!w,zB[j]=yb)J(w==1,zH[j]=yh)J(w==2,zI[j]=yi)E(zL[j]=yl))
...
A1(unq,XT(P(xn<2,x)P(xtB||xtC,C a[256];Ms(a,0,SZ a);Ay=an(xt,0);i(xn,I(!a[xc],a[xc]=1;y=apc(y,xc)))x(y))
          XS(cSI(unq(cSI(x))))K("{x@i@<i@:&@[;0;:;1]@~~':x@i:<x}",x))
       XM(unq(val(x)))Xz(Ln=gl(x);P(n<0,ed0())rndD(n))et1(x))
...
SN A cZ(Ax,Ct)_(P(xt==t,x)P(Tz[xt]==Tz[t],AT(t,mut(x)))
 x=N(cI(x));Xt(t==tC?ac(xv):x)Ay=an(t,xn);xe(T(&cIB,cIH,0,0,0,cIB)[t-tB](xV,yV,yn);y))
