...
SN V cGH(OV*RES a,V*RES b,N n)_(O G*p=a;H*r=b;i(n+15&-16,r[i]=p[i]))
SN V cGI(OV*RES a,V*RES b,N n)_(O G*p=a;I*r=b;i(n+ 7& -8,r[i]=p[i]))
SN V cHG(OV*RES a,V*RES b,N n)_(O H*p=a;G*r=b;i(n+31&-32,r[i]=p[i]))
SN V cHI(OV*RES a,V*RES b,N n)_(O H*p=a;I*r=b;i(n+ 7& -8,r[i]=p[i]))
SN V cHL(OV*RES a,V*RES b,N n)_(O H*p=a;L*r=b;i(n+ 3& -4,r[i]=p[i]))
SN V cIG(OV*RES a,V*RES b,N n)_(O I*p=a;G*r=b;i(n+ 7& -8,r[i]=p[i]))
SN V cIH(OV*RES a,V*RES b,N n)_(O I*p=a;H*r=b;i(n+ 7& -8,r[i]=p[i]))
SN V cIL(OV*RES a,V*RES b,N n)_(O I*p=a;L*r=b;i(n+ 3& -4,r[i]=p[i]))
SN V cLG(OV*RES a,V*RES b,N n)_(O L*p=a;G*r=b;i(n+ 3& -4,r[i]=p[i]))
SN V cLH(OV*RES a,V*RES b,N n)_(O L*p=a;H*r=b;i(n+ 3& -4,r[i]=p[i]))
SN V cLI(OV*RES a,V*RES b,N n)_(O L*p=a;I*r=b;i(n+ 3& -4,r[i]=p[i]))
SN V cLF(OV*RES a,V*RES b,N n)_(O L*p=a;F*r=b;i(n+ 3& -4,r[i]=p[i]==NL  ?NF:p[i]))
SN V cFL(OV*RES a,V*RES b,N n)_(O F*p=a;L*r=b;i(n+ 3& -4,r[i]=p[i]!=p[i]?NL:p[i]))
X1(flp,Rt(enl(enl(x)))R_(enl(x))RM(A y=kv(&x);am(x,y))
 ...
 RA(N m=xn;L n=cfm(xA,m|!m);P(n==-1,enl(x))P(n<0,el1(x))C t=_t(xx);I(t<tM&&t-tE,i(m,A y=xa;B(yt-t,t=0)))E(t=0)A y=aA(n);
  P(!t,i(n|!n,A z=aA(m);j(m,zA[j]=ii(xA[j],i))I(!zn,zx=mkn(zx))ya=sqz(z))x(0);I(!yn,yx=mkn(yx))y)
  I w=Tw[t];j(n|!n,A z=yA[j]=an(m,t);S4(w,i(m,zg=_G(xa)[j]),i(m,zh=_H(xa)[j]),i(m,zi=_I(xa)[j]),i(m|!m,zl=_L(xa)[j])I(w==4,I(!m,zx=mkn(_R(zx)))yA[j]=sqz(mRa(z)))))
  x(0);I(!n,yx=mkn(yx))y))
...
X1(rev,Rm(A y=kv(&x);am(rev(x),rev(y)))RM(A y=kv(&x);aM(x,e1f(rev,y)))Rt(x)RE(rev(gZ(x)))
 R_(P(xn<2,x)x=mut(x);N n=xn;I w=xw;S4(w,i(n>>1,SWP(xg,xG[n-1-i])),i(n>>1,SWP(xh,xH[n-1-i])),i(n>>1,SWP(xi,xI[n-1-i])),i(n>>1,SWP(xl,xL[n-1-i])))x))
A1(typ,x(as(TS[xt])))
A1(len,x(az(xN)))
...
X1(cA,RA(x)Rt(aA1(x))Rm(et1(x))R_(N n=xN;A y=aA(n);i(n|!n,ya=ii(x,i))x(0);I(!n,yx=mkn(yx))y))
