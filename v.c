...
X1(flp,Rt(enl(enl(x)))R_(enl(x))RM(A y=kv(&x);am(x,y))RB(flp(cG(x)))
 ...
 RA(U m=xn;L n=cfm(xA,m|!m);P(n==-1,enl(x))P(n<0,el(x))C t=_t(xx);I(t<tM&&t-tE,F(m,A y=xa;B(yt-t,t=0)))E(t=0)A y=aA(n);
  P(!t,F(n|!n,A z=aA(m);j(m,zA[j]=ii(xA[j],i))I(!zn,zx=mkn(zx))ya=sqz(z))x(0);I(!yn,yx=mkn(yx))y)
  U w=Tw[t]-3;j(n|!n,A z=yA[j]=an(m,t);S4(w,F(m,zg=_G(xa)[j]),F(m,zh=_H(xa)[j]),F(m,zi=_I(xa)[j]),F(m|!m,zl=_L(xa)[j])I(TR(t),I(!m,zx=mkn(_R(zx)))yA[j]=sqz(mRa(z)))))
  x(0);I(!n,yx=mkn(yx))y))
...
X1(whr,Ril(whr(enl(x)))RA(K1("{$[`A~@x;(,&#'*'x),,'/x@\\:!0|/#'x:o'x;,&x]}",x))Rm(A y=kv(&x);x(x1(Nx(whr(y)))))RE(whr(gZ(x)))R_(et(x))
 ...
 RGHIL(L m=xn,n=addfZ(0,x);P(n<0||minfZ(0,x)<0,ed(x))C t=tZ(m-!!m);P(t>tI,ez(x))A y=an(n,t);I w=xw-3;
  Mx(S4(t-tG,{G*r=yV;S4(w,F(m,j(xg,*r++=i)),F(m,j(xh,*r++=i)),F(m,j(xi,*r++=i)),F(m,j(xl,*r++=i)))},
             {H*r=yV;S4(w,F(m,j(xg,*r++=i)),F(m,j(xh,*r++=i)),F(m,j(xi,*r++=i)),F(m,j(xl,*r++=i)))},
             {I*r=yV;S4(w,F(m,j(xg,*r++=i)),F(m,j(xh,*r++=i)),F(m,j(xi,*r++=i)),F(m,j(xl,*r++=i)))},))y))
