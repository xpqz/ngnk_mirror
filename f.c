...
S A1(fBN,Q(xtB)Bv=0;i(xn,v|=xb)P(!(v>> 7&1),x)Ay=aL(xn);Mx(i(xn,yl=xb<0?NL:xb))y)
S A1(fHN,Q(xtH)Hv=0;i(xn,v|=xb)P(!(v>>15&1),x)Ay=aL(xn);Mx(i(xn,yl=xh<0?NL:xh))y)
S A1(fIN,Q(xtI)Iv=0;i(xn,v|=xb)P(!(v>>31&1),x)Ay=aL(xn);Mx(i(xn,yl=xi<0?NL:xi))y)
S L t[256];
S X1(fndBx,RmMA(e1f(fndBx,x))Rzc(Lv=gl(x);az(v==(C)v?t[(UC)v]:NL))RE(fndBx(gZ(x)))R_(fN(x))
 RZC_E(Nm=xn;Az=aL(m);Mx(S4(xw,i(m,zl=t[(UC)xc]),i(m,Hv=xh;zl=v==(C)v?t[(UC)v]:NL),i(m,Iv=xi;zl=v==(C)v?t[(UC)v]:NL),i(m,Lv=xl;zl=v==(C)v?t[(UC)v]:NL)))z))
...
X2(fnd,/*01*/
 ...
 R5(tH,tI,tL,tF,tS,
  ...
  P(xt==TT[yt]||xtZ&&ytzZ,
   ...
   Nm=xn,n=yn;Az=aL(n);My(S4(yw,i(n,zl=f(xV,m,yb)),i(n,zl=f(xV,m,yh)),i(n,zl=f(xV,m,yi)),i(n,zl=f(xV,m,yl))))z)
  fN(y)))
