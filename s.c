...
A1(kst,
 SW(xt,
  R(tc,kss(enl(x)))
  R(tm,Ay=kv(&x);cat11(apc(kl(x),'!'),kr(y)))
  R(tM,pre('+',kst(flp(x))))
  R(to,x(_R(xx)))
  R(tp,x=val(x);Ay=xx,z=xy;
  ...
  R(tq,raz(ek(val(x))))
  R(tr,x=val(x);Ay=_R(xy);cat11(kw(fir(x)),str(y)))
  R(ts,x=str0(str(x));I(xC+xn-pID(xC),x=kst(x);I(*xC==',',x=drp(1,x)))pre('`',x))
  R(tu,apc(kst(au^av^x),':'))
  R(tv,P(x==PLH,oC)Ay=aCn(vc+xv,1);xv<20?y:apc(y,':'))
  R(tw,aCn(&"':/:\\:"[xv%3*2],1+xv/3))
  R(tE,Lij x(0);x=pre('!',str(az(j)));i?cat11(apc(str(az(i)),'_'),x):x)
  R3(ti,tl,tf,str(x)))
 P(xn==1,pre(',',kr(fir(x))))
 SW(xt,
  R(tA,P(!xn&&!(_t(xx)==tC&&!_n(xx)),cat11(aCz("0#,"),kst(x(_R(xx)))))par(jc(';',ek(x))))
  R(tC,kss(x))
  R5(tB,tH,tI,tL,tF,xn?jc(32,e1f(str,x)):x(aCz(xtF?"0#0n":"!0")))
  R(tS,xn?raz(ek(x)):x(aCz("0#`"))))
 aCz("???"))
