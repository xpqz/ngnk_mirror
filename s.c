...
S C*S_8(C*s,U v)_(i(4,MC(s+6-2*i,q+v%100*2,2);v/=100)s+8) S C*s_8(C*s,U v)_(Cb[8],*p=b+6;W(1,MC(p,q+v%100*2,2);v/=100;B(!v)p-=2)p+=*p=='0';In=b+8-p;MC(s,p,n)+n)
S C*S16(C*s,W v)_(S_8(S_8(s,v/(U)1e8),v%(U)1e8))        S C*s16(C*s,W v)_(v<(U)1e8?s_8(s,v):S_8(s_8(s,v/(U)1e8),v%(U)1e8))
S C*su(C*s,W v)_(v<(W)1e16?s16(s,v):S16(s_8(s,v/(W)1e16),v%(W)1e16))C*sl(C*s,Lv)_(I(v<0,P(v==NL,Mh(s,"0N"))v=-v;*s++='-')su(s,v))
// github.com/ulfjack/ryu (apache2|boost license)
#if defined(__SIZEOF_INT128__)
 S W msh(W m,O W*a,Ii)_(__uint128_t g=m;((g**a>>64)+g*a[1])>>(i-64))S W msha(W m,O W*a,Ii,W*v,W*w,I sh)_(*v=msh(m+2,a,i);*w=msh(m-1-sh,a,i);msh(m,a,i))
#else
 ...
 S W msha(W m,O W*a,Ii,W*v,W*w,I sh)_(m>>=1;W t,l=mult(m,*a,&t),h,m0=t+mult(m,a[1],&h);h+=m0<t;W l2=l+*a,m2=m0+a[1]+(l2<l),h2=h+(m2<m0);*v=shr(m2,h2,i-65);
...
S V pws5(W NOP(*a)[2],In,L u,Lv,L w){W x[]={u,v,w},y[3],c;i(n,MC(a+i,x+1,16);shr3(y,x,2);c=0;i(3,x[i]=addcll(x[i],y[i],c,&c))I(x[2]>>61,shr3(x,x,1)))}
...
X1(str,Rilf(Ay=aC(24);TY(sf)*f=xtf?sf:sl;AN(f(yC,gl(x))-yC,y))Rs(Lv=xv;aCz(qs(&v)))Rc(enl(x))RU(kst(x))R_(e1f(str,x)))
V hexC(Q s,U n,C*r){S Qq="0123456789abcdef";i(n,Cc=*s++;*r++=q[(UC)c>>4];*r++=q[c&15])}
X1(hex,RmMA(e1f(hex,x))RC(Ay=aC(2*xn);Mx(hexC(xV,xn,yV))y)R_(et1(x)))
SN V unh0(C*q){MS(q,-1,256);i(10,q['0'+i]=i)i(6,q['A'+i]=q['a'+i]=10+i)}
A unhC(Q s,U n)_(P(n&1,el0())S C q[256];I(!*q,unh0(q))Cv=0;i(n,v|=q[s[i]])P(v<0,ed0())Ax=aC(n>>1);i(xn,xc=q[*s]<<4|q[s[1]];s+=2)x)
...
X1(kst,R_(aCz("???"))
 ...
 Rq(raz(ek(val(x))))
 Rr(x=val(x);Ay=_R(xy);cat11(kw(fir(x)),str(y)))
 Ru(apc(kst(au^av^x),':'))
 Rv(P(x==GAP,oC)Ay=aCn(vc+xv,1);xv<20?y:apc(y,':'))
