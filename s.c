...
C*sf(C*s,L d)_(UL x=d,m=x<<12>>12;I e=x<<1>>53;I(x>>63,*s++='-')P(e==2047,P(!m,Mh(s,"0w"))s=sl(s,m^1ll<<51);*s++='n';s)P(!m&&!e,Mh(s,"0.0"))
...
X1(kst,
 RE(kst(gZ(x)))
// RE(Lij x(0);x=pre('!',str(az(j-i)));i?cat11(apc(str(az(i)),'+'),x):x)
 ...
 Ru(apc(kst(au^av^x),':'))
 Rv(P(x==PLH,oC)Ay=aCn(vc+xv,1);xv<20?y:apc(y,':'))
