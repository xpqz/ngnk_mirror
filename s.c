...
A1(kst,
 ...
 XM(pre('+',kst(flp(x))))
 Xm(Ay=kv(&x);cat11(apc(ksl(x),'!'),ksr(y)))
 Xzf(str(x))
 Xs(x=str0(str(x));I(xC+xn-pID(xC),x=kst(x);I(*xC==',',x=drp(1,x)))pre('`',x))
 Xo(x(_R(xx)))
 Xp(x=val(x);Ay=xx,z=xy;
  P(xn==3&&((ytv&&y-av)||ytr)&&xy-PLH&&xz==PLH,z=ksl(zR);y=kst(fir(x));cA9(zC[zn-1])&&(cA9(*yC)||*yC==':'||*yC=='-')?cts(cat11(apc(y,'['),z),";]",2):cat11(z,y))
  x=ek(x);Nm=_n(xx);x=apc(jc(';',x),']');Q(xr==1);xC[m]='[';x)
 Xq(raz(ek(val(x))))
 Xr(x=val(x);Ay=_R(xy);cat11(ksw(fir(x)),str(y)))
