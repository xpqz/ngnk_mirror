#define _GNU_SOURCE // ngn/k, (c) 2019-2024 ngn, GNU AGPLv3 - https://codeberg.org/ngn/k/raw/branch/master/LICENSE
...
S I die(O C*s)_(I e=errno;wrZ(1,"ERR: ");wr(1,s,SL(s));wrZ(1," -> ");s=strerror(e);wr(1,s,SL(s));wrZ(1,"\n");exit(e);0)
S O C*m[][2]={{"html","text/html"},{"js","application/javascript"},{"css","text/css"},{"ogg","application/ogg"},{"wasm","application/wasm"}};
S O C*mime(O C*x)_(i(L(m),P(!strcmp(x,*m[i]),m[i][1]))(V*)0)
S V r404(I f){wrZ(f,"HTTP/1.1 404 Not Found\nContent-Length:4\nConnection:close\nContent-Type:text/html\n\n404\n");}
S V ap(C**p,O C*s){I n=SL(s);MC(*p,s,n);*p+=n;}
S O C*web(I f)_(C b[4096];I r=read(f,b,SZ b-1);P(r<=0,"read failed")b[r]=0;*SC0(b,10)=0;*SC0(b,13)=0;
 wr(1,b,SL(b));wrZ(1," - ");P(strncmp(b,"GET ",4),"not GET")C*s=b+4;s=sk(s,32);s+=*s=='/';*SC0(s,32)=0;
 I(!*s,s="index.html")O C*d=strrchr(s,'.'),*m=d?mime(d+1):0;I(!m,m="text/plain")
...
I main(){struct sockaddr_in a;a.sin_family=AF_INET;a.sin_addr.s_addr=0;a.sin_port=htons(port);
 ...
 W(1,UI n=SZ a;I f=Q(accept(l,(V*)&a,&n));O C*r=web(f);I(r,wr(1,r,SL(r));wrZ(1,"\n");r404(f))close(f))}
