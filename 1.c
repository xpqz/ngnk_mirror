...
X1(neg,RE(neg(gZ(x)))Rzc(az(-gl(x)))Rf(af(-gf(x)))RC(neg(cG(x)))RmMA(e1f(neg,x))R_(et1(x))
 RZF_E(N n=xn;I(xw<3&&minfZ(0,x)==-1<<(8<<xw)-1,x=cT[tH+xw](x))A y=xr>1?an(n,xt):x,z=x-y?x:au;n=((n<<xw)+31&~31)>>xw;
...
X1(flr,RmMA(e1f(flr,x))RcC(K1("{`c$x+32*~\"A[\"'x}",x))RsS(cS(flr(str(x))))RzZ(x)RfF(A y=an(xn,xt+tl-tf);Mx(i(yn,yl=xf<0?(L)xf-(xf<(L)xf):(L)xf))y)R_(et1(x)))
