...
X1(neg,RE(neg(gZ(x)))Rzc(az(-gl(x)))Rf(af(-gf(x)))RC(neg(cB(x)))RmMA(e1f(neg,x))R_(et1(x))
 RZF_(Nn=xn;I(xw<3&&minfZ(0,x)==-1<<(8<<xw)-1,x=cT[tH+xw](x))Ay=xr>1?an(n,xt):x;
  SW(xt,C(tB,i(PD(n,xB),yb=-xb))C(tH,i(PD(n,xH),yh=-xh))C(tI,i(PD(n,xI),yi=-xi))C(tL,i(PD(n,xL),yl=-xl))D(i(PD(n,xF),yf=-xf)))x-y?x(y):y))
...
X1(flr,RmMA(e1f(flr,x))RcC(K1("{`c$x+32*~\"A[\"'x}",x))RsS(cS(flr(str(x))))RzZ(x)RfF(Ay=an(xn,xt+tl-tf);xe(i(yn,yl=xf<0?(L)xf-(xf<(L)xf):(L)xf)y))R_(et1(x)))

#define h(k,f) X1(k,RfF(Ay=xr-1?an(xn,xt):x;i(PD(xn,xF),yf=f(xf));x-y?x(y):y)RmMA(e1f(k,x))R_(k(N(cF(x)))))
