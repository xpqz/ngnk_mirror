...
X1(neg,RE(neg(gZ(x)))Rzc(az(-gl(x)))Rf(af(-gf(x)))RC(neg(cB(x)))RmMA(e1f(neg,x))R_(et1(x))
 RZF_E(Nn=xn;I(xw<3&&minfZ(0,x)==-1<<(8<<xw)-1,x=cT[tH+xw](x))Ay=xr>1?an(n,xt):x;n=((n<<xw)+31&~31)>>xw;
  X(C(tB,i(n,yb=-xb))C(tH,i(n,yh=-xh))C(tI,i(n,yi=-xi))C(tL,i(n,yl=-xl))D(i(n,yf=-xf)))x-y?x(y):y))
X1(not,RmMA(e1f(not,x))RU(x(ai(x==au)))R_(eql(xtsS?as(0):ai(0),x)))
X1(nul,RmMA(e1f(nul,x))RU(x(ai(x==au)))RF(Ay=aB(xn);Mx(i(xn,yb=xf!=xf))y)Rf(x(ai(*xF!=*xF)))R_(eql(cn[xt],x)))
