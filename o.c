...
S L o0(L v)_(t(v-o))S V of0LL(O L*a,L*r,Nn){i(n+3&~3,r[i]=o0(a[i]))}A1(of0,Q(xtlL);of_(x,0))
S L o1(L v)_(t(v)+o)S V of1LL(O L*a,L*r,Nn){i(n+3&~3,r[i]=o1(a[i]))}A1(of1,Q(xtfF);of_(x,1))
...
I qA(Ax,Ay/*00*/)_(I v=TS[xt]-TS[yt];P(v,v)
 X(Rz(ql(gl_(x),gl_(y)))
   Rf(qf(*xF,*yF))
   Rs(L v=xv,w=yv;strcmp(qs(&v),qs(&w)))
   RT(i(MIN(xn,yn),Az=ii(x,i),u=ii(y,i);I d=qA(z,u);mr(z(u));P(d,d))ql(xn,yn))
   Ropqr(x=str(xR);y=str(yR);I r=qA(x,y);x(y(0));r)
   R_(ql(x,y)))0)
S I*ascZ(O UC*v,UC*g,I*a,I*b,In,I w)_(U c[257];tilV(a,0,n,2);j(w,MS(c,0,SZ c);i(n,g[i]=v[w*a[i]+j])i(n,c[g[i]+1]++)I(c[1+*g]-n,i(255,c[i+1]+=c[i])i(n,b[c[g[i]]++]=a[i])SWP(b,a)))a)
...
X1(asc,Rt(opn(x))Rm(grdm(x,asc))RM(K1("{(!#x){x@<y x}/|.+x}",x))RS(asc(str(x)))RF(asc(of1(x)))RA(P(xn-(I)xn,ez1(x))ascA(x))RE(Lij x(0);aE(0,j-i))
 RGC(P(xn-(I)xn,ez1(x))Nn=xn;I c[257]={},*c129=c+129;i(n,c129[xg]++)i(256,c[i+1]+=c[i])Ay=aI(n);I*c128=c+128;Mx(i(n,yI[c128[xg]++]=i))cT[tZ(n-1)](y))
...
X1(grp,Rz(K1("=/:/2#,!:",x))Rm(Ay=kv(&x);y=Nx(grp(y));yy=x(i1(x,yy));y)R_(et1(x))
 RGC(A r[  256]={};UC b[  256];U nb=0;U c[  256]={};i(xn,UC v=xg;I(!c[v]++,b[nb++]=v))Az=aA(nb);i(nb,za=r[b[i]]=aI(c[b[i]]))I(!nb,*zA=oG)MS(c,0,SZ c);i(xn,UC v=xg;_I(r[v])[c[v]++]=i)x(am(aV(xt,nb,b),z)))
 RH( A r[65536]={};UH b[65536];U nb=0;U c[65536]={};i(xn,UH v=xh;I(!c[v]++,b[nb++]=v))Az=aA(nb);i(nb,za=r[b[i]]=aI(c[b[i]]))I(!nb,*zA=oG)MS(c,0,SZ c);i(xn,UH v=xh;_I(r[v])[c[v]++]=i)x(am(aV(xt,nb,b),z)))
...
X1(unq,RM(en1(x))Rm(unq(val(x)))RE(x)RS(cSI(unq(cSI(x))))Rz(rndF(gl(x)))R_(et1(x))RGC(C a[256]={},r[256],t=xt;Nn=0;Mx(i(xn,UC v=xg;I(!a[v],a[v]=1;r[n++]=v)))aV(t,n,r))
