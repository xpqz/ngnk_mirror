...
X1(grp,Rz(K1("=/:/2#,!:",x))Rm(A y=kv(&x);y=Nx(grp(y));yy=x(i1(x,yy));y)R_(et1(x))
...
X1(unq,RM(en1(x))Rm(unq(val(x)))RE(x)RS(cSI(unq(cSI(x))))Rz(rndF(gl(x)))R_(et1(x))RGC(C a[256]={},r[256],t=xt;N n=0;Mx(i(xn,UC v=xg;I(!a[v],a[v]=1;r[n++]=v)))aV(t,n,r))
