...
S A0(js,UC c;C*p=++s;Nn=0;
 W((c=*p)-34,P(c<32,s=p;0)p++;I(c==92,c=*p++;P(c<32,s=p;0)n++;I(c=='u',Lv=ju(p);P(v<0,s=p;0)p+=4;n+=4-(v>127)-(v>2047))))
 Ax=aC(p-s-n);p=s;C*r=xC;
 W((c=*p++)-34,I(c==92,c=*p++;
  I(c-'u',Ij=si(EJ,c);P(j>=SZ EJ,s=p-1;x(0))c=JE[j])
  E(Iv=ju(p);p+=4;I(v<128,c=v)J(v<2048,*r++=192|v>>6;c=128|63&v)E(*r++=224|v>>12;*r++=128|63&v>>6;c=128|63&v)))*r++=c)
 s=p;x)
S A0(ja,s++;Ax=oA;P(jw()==']',s++;x)W(1,xq(Nx(jx()));P(*s==']',s++;x)Nx(*s==44);s++)0)
S A0(jo,s++;Ax=oS,y=oA;Cc=jw();P(c=='}',s++;am(x,y))
 W(c==34,Az=js();B(!z)xq(cS(z));B(jw()-':')s++;z=jx();B(!z)yq(z);P(*s=='}',s++;am(x,y))B(*s-44)s++;c=jw())x(y(0)))
...
S V JC(Qp,Nn){I(!*t,it())*s++=34;UC c;
 i(n,SW(t[c=*p++],C(0,*s++=c)C(1,*s++=92;*s++=EJ[si(JE,c)])D(Mc(s,"\\u00",4);s+=4;*s++=HEX[c>>4];*s++=HEX[c&15])))
 *s++=34;}
