...
I si(Qs,Iv)_(SC0(s,v)-(C*)s)S F p10(In)_(Fv=1;i(n,v*=10)v)S Q pw(Qs)_(W(*s==32,s++)s)
S I id(UC c)_(cAz(c)|((c|1)==0xd1)<<1)S I id1(Cc)_(id(c)|c09(c))Q pID(Qs)_(W(1,Ii=id1(*s);B(!i)s+=i)s)
...
S A pT(C*v)_(Ax=N(pt(v));
 W(1,Cc=*s;Ii=si("'/\\[",c);P(i>3,x)s++;
  ...
  E(I u=*s==':';s+=u;x=aA2(aw+i+3*u,x);*v=1))x)
S A pe(Ax,C*v)_(s=pw(s);Cc=*s;I(c=='/'&&(s==s0||s[-1]==32||s[-1]==10),W((c=*++s)&&c-10))
 ...
 Az=pe(0,v);P(!z,y(x?x(0):0))P(z==PLH,*v=1;P(!x,y)Yu(ep1(x))AO(o,aA3(y,x,z)))*v&=y!=av;I(!x,y=pm(y))
 *v?aA3(aw,x?AO(o,aA3(y,x,PLH)):y,z):AO(o,x?aA3(y,x,z):aA2(pm(y),z)))
