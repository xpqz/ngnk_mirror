...
U si(Qs,Cv)_(strchrnul(s,v)-(C*)s)                                                                  //find char (string index)
...
W pu(Q*p)_(Qs=*p;Lv=0;Cc=*s;W(C09(c),v=10*v+c-'0';c=*++s)*p=s;v)                                    //parse long int unsigned
...
W pf(Q*p)_(Im=**p=='-';(*p)+=m;Lv=(W)m<<63|pfu(p);(*p)+=**p=='f';v)                                 //parse float
