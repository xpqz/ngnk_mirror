...
S Qs,s0;S U k;S A pb(A,C);                                                                          //parser state (s:current pointer, s0:start of source, k:implicit arg counter)
...
S B num(Qs)_(C09(s[*s=='-']))                                                                       //is number start?
...
W pu(Q*p)_(Qs=*p;L v=0;Cc=*s;W(C09(c),v=10*v+c-'0';c=*++s)*p=s;v)                                   //parse long int unsigned
L pl(Q*p)_(Im=**p=='-';*p+=m;(1-2*m)*pu(p))                                                         //parse long int
S W pfu(Q*p)_(W v=pu(p);Qs=*p;Cc=*s;P(c=='w',(*p)++;WFL)P(c=='n',(*p)++;v^NFL)I e=0;                //parse float unsigned
...
W pf(Q*p)_(Im=**p=='-';(*p)+=m;L v=(W)m<<63|pfu(p);(*p)+=**p=='f';v)                                //parse float
S A0(pZ,Qp=s;W(*p-'0'<2u,p++)P(*p=='b',Ax=aG(p-s);i(xn,xg=*s++&1)s++;x)                             //parse ints
 Ax=oL;W(1,L v=pl(&s);I(!v&&*s=='N',v=NL;s++)x=apv(x,&v);Qp=pw(s);B(p==s||!num(p))s=p)sqzZ(x))
...
S X1(pm,Rv(x^au^av)RA(I(xx==aw,x=mut(x);xA[xn-1]=pm(xA[xn-1]))x)Rs(L v=xv;Qs=qs(&v);Nn;P(*s>>7&&s[(n=SL(s))-1]-':',C b[8];MC(b,s,n);b[n]=':';b[n+1]=0;sym(b))x)R_(x)) //monadify
...
S A pe(Ax,C*v)_(s=pw(s);Cc=*s;                                                                      //parse expression
 ...
 UH o=s-s0;C b=0;Ay=pT(&b);P(!y,x?x(0):0)P(y==GAP,x?x:y)
...
S A pb(Ax,Cc)_(x=x?aA1(x):oA;                                                                       //parse body (sequence of ;-separated expressions)
 W(1,C v=0;Ay=Nx(pe(0,&v));xq(c-']'&&y==GAP?au:y);P(y==GAP&&c==')',ep1(x))B(*s-';'&&*s-10)B(c==10&&*s==10)s++)
