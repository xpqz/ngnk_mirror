...
S A1(cf,XA(P(xx==MKL,i(xn,Ay=xa;YsSA(x))aA1(drp(1,x)))P(xn==2?f2(xx,xy):xn==3?f3(xx,xy,xz):0,qt(N(val(x))))
 Ay=rsh(xn,au);/*preserve src offsets*/i(xn,ya=cf(xa);xa=au;P(!ya,die("cf")))AO(xo,x(y)))x)
#define IND(a,b,i,d) ((i)-(b)<(UI)LEN(a)?(a)[(i)-(b)]:(d))//a:array,b:baseIndex,i:index,d:default
S I mxs(Ii,I s)_(I r=s;W(1,UC c=b[i++];Q(s>=0)r=max(r,s);P(!c,r)s+=IND(ds,ba,c,c>=bc||c-bg<8u||c-bd<8u?1:c-bs<8u||c-bv<32u?-1:0)+((c==bL)-(c==bl||c==ba||c==bP))*b[i];
 i+=IND(di,ba,c,0)+(c==bj)*b[i];I(c==bz,r=max(r,mxs(i+b[i-1],s))))r)
