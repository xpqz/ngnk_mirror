...
#define hc(a) N(cc(a,o));
#define OK -1
#define Nr(a...) {I r_=cr(a);P(r_-OK,r_);}
#define Nl(a...) {I r_=cl(a);P(r_-OK,r_);}
#define fl uA[3]
S A u,cr(A,I);S UC*b,*m;S I nb,lu[8];
SN I ig(Lv)_(I(*gp,Qs=qs(&v);I(!strchr(s,'.'),Nm=SL(gp),n=SL(s);P(m+n+3>SZ gp,-1)gp[m]='.';MC(gp+m+1,s,n+1);v=(I)sym(gp);gp[m]=0))
 Li=fI(gk,gn,v);P(i>=0,i)P(gn-(UC)gn,-1)gk[gn]=v;gv[gn]=0;gn++)
SN I il(Iv)_(Li=fI(_V(fl),_n(fl),v);P(i>=0,lu[i]=nb;i)-1)
S I cc(Ax/*1*/,I o)_(Ay=u;Nn=yn,i=5;W(i<n,B(mtc_(x,ya),x=x(0))i++)I(x,P(bc+i-5>255,ez1(x))uq(x))h(bc+i-5)1)
S A cl(Ax,Ay/*00*/,I r)_(Q(xx==av||_t(xx)==tu)Iv=_v(xx),o=xo;
 YsS(I(yts,P(xx==av&&_n(fl),Li=il(yv);I(i<0,i=_n(fl);P(i>15,ez0())Iv=yv;fl=apv(fl,&v);lu[i]=nb)h(bs+i)I(r,h(bg+i))OK)Ii=il(yv);P(i>=0,h(bm)h(i)h(v)I(r,h(bg+i))OK))E(P(yn==1,o)y=jS(yR))
...
S I mxs(Ii,I s)_(I r=s;W(1,UC c=b[i++];Q(s>=0)r=MAX(r,s);P(!c,r)s+=(c-ba<L(ds)?ds[c-ba]:c>=bc||c-bg<8u||c-bd<8u?1:c-bs<8u||c-bv<32u?-1:0)+((c==bL)-(c==bl||c==ba||c==bP))*b[i];
 i+=(c-ba<L(di)?di[c-ba]:0)+(c==bj)*b[i];I(c==bz,r=MAX(r,mxs(i+b[i-1],s))))r)
S I shy(Ax/*0*/)_(!xtA?0:xn&&xx==PLH?shy(xA[xn-1]):xn==3&&(xx==av||_t(xx)==tu||(_t(xx)==ts&&cv(_v(xx))))&&_tsSA(xy))
...
AX(run,Q(xto)P(n-xk,er8(a,n))S I d;P(++d>2048,es8(a,n))UC*b=_V(xy),c,ns=*b++,nl=_n(xA[3]);A t[ns+nl],*s=t+ns,*l=s;MS(t,0,SZ t);MC(l,a,8*n);
 W((c=*b++),
