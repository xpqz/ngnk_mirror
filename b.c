...
S A cr(Ax/*0*/,B r)_(I o=xo;                                                                        //compile rvalue (x:tree,r:wantResult?)
 ...
 P(n>3&&(y==av||y==CST),n--;I p[n];A*a=xA;i(n&~1,Nr(*++a,1);h(i&1?bj:bz)p[i]=nb;h(0))               // $[x;y;z] cond
  ...
  i(n&~1,I d=(i&1?nb-1:p[i+1])-p[i];I(i&1,Ij=(n&~1)-1;W(i<j&&d>255,d=p[j]-1-p[i];j-=2))             //
   P(d>255,ez0())b[p[i]]=d)                                                                         //
  I(!r,h(bp))OK)                                                                                    //
 I(n==2&&y==FIR,Az=xy;I(ztA&&zn==2,Q(REV+1==ASC)Q(ASC+1==DSC)Q(LAS+1==IMN)Q(IMN+1==IMX)             // *|x      recognized idioms
...
S I mxs(Ii,I s)_(I r=s;                                                                             //max stack
 W(1,UC c=b[i++];Q(s>=0)r=MAX(r,s);P(!c,r)
     s+=(c>=bc||c-bg<16u||c-bd<16u?1:c-bs<16u||c-bv<32u?-1:ds[c])+
        ((c==bL)-(c==bl||c==ba||c==bP))*b[i];
     i+=di[c]+(c==bj)*b[i];
     I(c==bz,r=MAX(r,mxs(i+b[i-1],s))))r)
S B shy(Ax/*0*/)_(                                                                                  //is the last expression in the tree x an assignment?
