...
Z W b[M];Z U nb;//buf
X1(prng,Ru(aV(tL,4*M,s))REGHIL(P(xn-4*M,el(x))MC(s,xV,SZ s);nb=0;x(au))Ril(W v=gl(x);I(!v,v=now())i(4,j(M,s[i][j]=v=v*6364136223846793005+1442695040888963407/*knuth mmix*/))au)R_(et(x)))
Z V h(U x,U y){i(M,s[x][i]^=s[y][i])}
Z V r4(){nb=M;W t[M];i(M,b[i]=s[0][i]+s[3][i])i(M,t[i]=s[1][i]<<17)h(2,0);h(3,1);h(1,2);h(0,3);i(M,s[2][i]^=t[i])i(M,s[3][i]=(s[3][i]<<45|s[3][i]>>19))}//next 4*64 bits
...
Z A sh(U n)_(A x=an(n,tZ(n));S4(xw-3,i(n,I j=ri(i+1);xg=xG[j];xG[j]=i),i(n,I j=ri(i+1);xh=xH[j];xH[j]=i),i(n,I j=ri(i+1);xi=xI[j];xI[j]=i),ez(x))x)//shuffle
Z A rt(U n,C t)_(A x=an(n,t);i(((W)n<<Tw[t])+255>>8,r4();MC(xV+(i<<5),b,32))x)//roll n full-range (including negative) items of type t (int of a specific width or char)
